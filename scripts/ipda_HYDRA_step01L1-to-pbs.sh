...
Description: Write and submit PBS jobs for Step 01L1 (long-reads) of the
HYDRA pipeline (HYbrid De novo RNA Assembly pipeline).

Usage: bash ipda_HYDRA_step01L1-to-pbs.sh -i "path/to/input/files" -p "PBS stem" -e "email" -m INT -c INT -w "HH:MM:SS"
...
do
    case "${flag}" in
        ...
        ?) echo script usage: bash ipda_HYDRA_step01L1-to-pbs.sh -i path/to/input/files -p PBS stem -e email -m INT -c INT -w "HH:MM:SS" >&2
           exit;;
    esac
done
...
logfile=logfile_ipda_HYDRA_step01L1-to-pbs_${thislogdate}.txt
...
out_path_step01L1_NanoPlot="HYDRA_step01L1_quality-check-raw-files_NanoPlot_${thislogdate}"

## Create output directories
mkdir -p ${out_path_step01L1_NanoPlot}
...
date
echo "## Executing bash ipda_HYDRA_step01L1-to-pbs.sh"
...
echo
echo "## Output files saved to:       ${out_path_step01L1_NanoPlot}"
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; mkdir -p ${out_path_step01L1_NanoPlot}/${file}; f1="${path_file}.fastq.gz"; echo "NanoPlot -t ${ncpus} -o ${out_path_step01L1_NanoPlot}/${file} --fastq ${f1}" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
date ## Status of all user jobs (including HYDRA step 01L1 jobs) at
qstat -u "$user"
...
sed -i 's,${out_path_step01L1_NanoPlot},'"${out_path_step01L1_NanoPlot}"',g' "$logfile"
