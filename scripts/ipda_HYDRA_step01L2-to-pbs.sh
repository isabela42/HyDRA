...
Description: Write and submit PBS jobs for Step 01L2 (long-reads) of the
HYDRA pipeline (HYbrid De novo RNA Assembly pipeline).

Usage: bash ipda_HYDRA_step01L2-to-pbs.sh -i "path/to/input/files" -p "PBS stem" -e "email" -m INT -c INT -w "HH:MM:SS"
...
do
    case "${flag}" in
        ...
        ?) echo script usage: bash ipda_HYDRA_step01L2-to-pbs.sh -i path/to/input/files -p PBS stem -e email -m INT -c INT -w "HH:MM:SS" >&2
           exit;;
    esac
done
...
logfile=logfile_ipda_HYDRA_step01L2-to-pbs_${thislogdate}.txt
...
out_path_step01L2_NanoComp="HYDRA_step01L2_quality-check-raw-files_NanoComp_${thislogdate}"

## Create output directories
mkdir -p ${out_path_step01L2_NanoComp}
...
date
echo "## Executing bash ipda_HYDRA_step01L2-to-pbs.sh"
...
echo
echo "## Output files saved to:       ${out_path_step01L2_NanoComp}"
...
echo "NanoComp -t ${ncpus} --fastq ${fastq_files} --names ${names} -o ${out_path_step01L2_NanoComp}" >> ${pbs_stem}_${thislogdate}.pbs
...
date ## Status of all user jobs (including HYDRA step 01L2 jobs) at
qstat -u "$user"
...
sed -i 's,${out_path_step01L2_NanoComp},'"${out_path_step01L2_NanoComp}"',g' "$logfile"
