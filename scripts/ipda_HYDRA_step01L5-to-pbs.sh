...
logfile=logfile_ipda_HYDRA_step01L5-to-pbs_${thislogdate}.txt
...
out_path_step01L5_FastaSplitter="HYDRA_step01L5_size-split_FastaSplitter_${thislogdate}"
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "seqtk seq -a ${path_file}.fastq.gz > ${out_path_step01L5_FastaSplitter}/${file}.fasta" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "python ${fasta_splitter} -f ${out_path_step01L5_FastaSplitter}/${file}.fasta  -n 20000 -p ${out_path_step01L5_FastaSplitter}/${file}" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
