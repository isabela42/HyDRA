...
logfile=logfile_ipda_HYDRA_step02L1-to-pbs_${thislogdate}.txt
...
out_path_step02L1_Porechop="HYDRA_step02L1_trim-reads-adapters_Porechop_${thislogdate}"
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "porechop -i ${path_file}.fastq.gz --threads ${ncpus} -o ${out_path_step02L1_Porechop}/adapter-trimmed_${file}.fastq.gz" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
