...
Description: Write and submit PBS jobs for Step 02L2 (short-reads) of the
HYDRA pipeline (HYbrid De novo RNA Assembly pipeline).

Usage: bash ipda_HYDRA_step02L2-to-pbs.sh -i "path/to/input/files" -p "PBS stem" -e "email" -m INT -c INT -w "HH:MM:SS"

Resources used for pipeline development: -m 2 -c 2 -w "02:00:00"
...
-i <path/to/input/files>    Input TSV files including path FROM working
                            ...
                            Col1:
                            path/from/working/dir/to/HYDRA_step02L1_trim-reads-adapters_Porechop_DATE/stem
                            of .fastq.gz file and no full stops.

                            Col2:
                            path/from/working/dir/to/HYDRA_step02L1_trim-reads-adapters_Porechop_DATE/stem.fastq.gz
...
do
    case "${flag}" in
        ...
        ?) echo script usage: bash ipda_HYDRA_step02L2-to-pbs.sh -i path/to/input/files -p PBS stem -e email -m INT -c INT -w "HH:MM:SS" >&2
           exit;;
    esac
done
...
logfile=logfile_ipda_HYDRA_step02L2-to-pbs_${thislogdate}.txt
...
# source activate chopper-0.5.0
# commands
# conda deactivate
module_chopper="chopper-0.5.0"
...
out_path_step02L2_Chopper="HYDRA_step02L2_trim-quality-headcrop_Chopper_${thislogdate}"

## Create output directories
mkdir -p ${out_path_step02L2_Chopper}
...
date
echo "## Executing bash ipda_HYDRA_step02L2-to-pbs.sh"
...
echo
echo "## Output files saved to:       ${out_path_step02L2_Chopper}"
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "source activate ${module_chopper}" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "#................................................" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo 'echo "## Run Chopper at" ; date ; echo' >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "gunzip -c ${path_file}.fastq.gz | chopper -q 10 --headcrop 12 --threads ${ncpus} | gzip > ${out_path_step02L2_Chopper}/quality-trimmed_${file}.fastq.gz" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
date ## Status of all user jobs (including HYDRA step 02L2 jobs) at
qstat -u "$user"
...
sed -i 's,${module_chopper},'"${module_chopper}"',g' "$logfile"
sed -i 's,${miniconda3},'"${miniconda3}"',g' "$logfile"
sed -i 's,${out_path_step02L2_Chopper},'"${out_path_step02L2_Chopper}"',g' "$logfile"
