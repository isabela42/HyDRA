...
Description: Write and submit PBS jobs for Step 02L3 (long-reads) of the
HYDRA pipeline (HYbrid De novo RNA Assembly pipeline).

Usage: bash ipda_HYDRA_step02L3-to-pbs.sh -i "path/to/input/files" -p "PBS stem" -e "email" -m INT -c INT -w "HH:MM:SS"
...
-i <path/to/input/files>    Input TSV files including path FROM working
                            ...
                            Col1:
                            path/from/working/dir/to/HYDRA_step02L2_trim-quality-headcrop_Chopper_DATE/stem
                            of .fastq.gz file and no full stops.

                            Col2:
                            path/from/working/dir/to/HYDRA_step02L2_trim-quality-headcrop_Chopper_DATE/stem.fastq.gz
...
do
    case "${flag}" in
        ...
        ?) echo script usage: bash ipda_HYDRA_step02L3-to-pbs.sh -i path/to/input/files -p PBS stem -e email -m INT -c INT -w "HH:MM:SS" >&2
    esac
done
...
logfile=logfile_ipda_HYDRA_step02L3-to-pbs_${thislogdate}.txt
...
# Python 3.9.13:
# <https://cutadapt.readthedocs.io/en/stable/recipes.html#trim-poly-a-tails>
module_python=python/3.9.13
...
out_path_step02L3_Cutadapt="HYDRA_step02L3_trim-reads-polyA_Cutadapt_${thislogdate}"

## Create output directories
mkdir -p ${out_path_step02L3_Cutadapt}
...
date
echo "## Executing bash ipda_HYDRA_step02L3-to-pbs.sh"
...
echo
echo "## Output files saved to:       ${out_path_step02L3_Cutadapt}"
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "#................................................" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo 'echo "## Run Cutadapt at" ; date ; echo' >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "cutadapt --poly-a ${path_file}.fastq.gz -o ${out_path_step02L3_Cutadapt}/polyA-trimmed_${file}.fastq.gz" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
#cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "cutadapt --poly-a ${path_file}.fastq.gz > ${out_path_step02L3_Cutadapt}/polyA-trimmed_${file}.fastq" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
date ## Status of all user jobs (including HYDRA step 02L3 jobs) at
qstat -u "$user"
...
sed -i 's,${out_path_step02L3_Cutadapt},'"${out_path_step02L3_Cutadapt}"',g' "$logfile"
