...
-i <path/to/input/files>    Input TSV files including path FROM working
                            ...
                            path/from/working/dir/to/raw/reads/stem_R1.f* (line 2)
...
merged-lanes.f*       Fastq.gz file for each stem R1 and R2 merged lanes.
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; f1="${path_file}_R1.f*"; f2="${path_file}_R2.f*"; echo "perl ${rcorrector} -t ${ncpus} -od ${out_path_step02S1_Rcorrector} -1 ${f1} -2 ${f2}" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; f1="${out_path_step02S1_Rcorrector}/${file}_R1.cor.fq.gz"; f2="${out_path_step02S1_Rcorrector}/${file}_R2.cor.fq.gz"; echo "bash ${reformat} in=${f1} in2=${f2}" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
