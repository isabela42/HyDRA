...
WARNING: This script will merge different stem_L00* fastq.gz file. If this is not desired, must change from line 333 onwards
or alternatively use the ipda_HYDRA_step02S1-nomerge-to-pbs.sh script.

Usage: bash ipda_HYDRA_step02S1-to-pbs.sh -i "path/to/input/files" -p "PBS stem" -e "email" -m INT -c INT -w "HH:MM:SS"

Resources used for pipeline development: -m 50 -c 15 -w "35:00:00"
...
-i <path/to/input/files>    Input TSV files including path FROM working
                            ...
                            of both L00*_R1.fastq.gz and L00*_R2.fastq.gz files in individual lines
                            and no full stops. This script will merge the different
                            stem_L00*_R1.fastq.gz together, as well as stem_L00*_R2.fastq.gz.
                            If this is not desired, must change from line 333 onwards.

                            Col2:
                            path/from/working/dir/to/raw/reads/stem_R1.fastq.gz (line 1)
                            path/from/working/dir/to/raw/reads/stem_R1.fastq.gz (line 2)
...
merged-lanes.fastq.gz       Fastq.gz file for each stem R1 and R2 merged lanes.
...
do
    case "${flag}" in
        ...
        ?) echo script usage: bash ipda_HYDRA_step02S1-to-pbs.sh -i path/to/input/files -p PBS stem -e email -m INT -c INT -w "HH:MM:SS" >&2
           exit;;
    esac
done
...
logfile=logfile_ipda_HYDRA_step02S1-to-pbs_${thislogdate}.txt
...
rcorrector="/working/lab_julietF/isabelaA/tools/rcorrector/run_rcorrector.pl"

# Reformat (BBTools suite):
# <https://jgi.doe.gov/data-and-tools/software-tools/bbtools/bb-tools-user-guide/>
reformat="/working/lab_julietF/isabelaA/tools/bbmap/reformat.sh"
...
out_path_step02S1_Rcorrector="HYDRA_step02S1_reads-correction_Rcorrector_${thislogdate}"

## Create output directories
mkdir -p ${out_path_step02S1_Rcorrector}
...
date
echo "## Executing bash ipda_HYDRA_step02S1-to-pbs.sh"
...
echo
echo "## Output files saved to:       ${out_path_step02S1_Rcorrector}"
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "#................................................" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "## Change to Working Directory" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "echo \"${rcorrector}\"" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "echo \"${reformat}\"" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "#................................................" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo 'echo "## Run gzip at" ; date ; echo' >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "zcat -c ${path_file}_L00*_R1.fastq.gz | gzip -c >> ${out_path_step02S1_Rcorrector}/${file}_merged-lanes_R1.fastq.gz" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "zcat -c ${path_file}_L00*_R2.fastq.gz | gzip -c >> ${out_path_step02S1_Rcorrector}/${file}_merged-lanes_R2.fastq.gz" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; f1="${out_path_step02S1_Rcorrector}/${file}_merged-lanes_R1.fastq.gz"; f2="${out_path_step02S1_Rcorrector}/${file}_merged-lanes_R2.fastq.gz"; echo "perl ${rcorrector} -t ${ncpus} -od ${out_path_step02S1_Rcorrector} -1 ${f1} -2 ${f2}" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; f1="${out_path_step02S1_Rcorrector}/${file}_merged-lanes_R1.cor.fq.gz"; f2="${out_path_step02S1_Rcorrector}/${file}_merged-lanes_R2.cor.fq.gz"; echo "bash ${reformat} in=${f1} in2=${f2}" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
date ## Status of all user jobs (including HYDRA step 02S1 jobs) at
qstat -u "$user"
...
sed -i 's,${rcorrector},'"${rcorrector}"',g' "$logfile"
sed -i 's,${reformat},'"${reformat}"',g' "$logfile"
sed -i 's,${out_path_step02S1_Rcorrector},'"${out_path_step02S1_Rcorrector}"',g' "$logfile"
