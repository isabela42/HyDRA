...
Usage: bash ipda_HYDRA_step02S2-to-pbs.sh -i "path/to/input/files" -p "PBS stem" -e "email" -m INT -c INT -w "HH:MM:SS"

Resources used for pipeline development: -m 1 -c 1 -w "02:00:00"
...
-i <path/to/input/files>    Input TSV files including path FROM working
                            ...
                            Col1:
                            path/from/working/dir/to/HYDRA_step02S1_reads-correction_Rcorrector_DATE/stem
                            of both _R1.cor.fq* and _R2.cor.fq* files in individual lines
                            and no full stops.

                            Col2:
                            path/from/working/dir/to/HYDRA_step02S1_reads-correction_Rcorrector_DATE/stem_R1.cor.fq* (line 1)
                            path/from/working/dir/to/HYDRA_step02S1_reads-correction_Rcorrector_DATE/stem_R2.cor.fq* (line 2)
...
do
    case "${flag}" in
        ...
        ?) echo script usage: bash ipda_HYDRA_step02S2-to-pbs.sh -i path/to/input/files -p PBS stem -e email -m INT -c INT -w "HH:MM:SS" >&2
           exit;;
    esac
done
...
logfile=logfile_ipda_HYDRA_step02S2-to-pbs_${thislogdate}.txt
...
# FUPER (FilterUncorrectabledPEfastq):
# <https://github.com/harvardinformatics/TranscriptomeAssemblyTools/blob/master/FilterUncorrectabledPEfastq.py>
fuper="/working/lab_julietF/isabelaA/tools/TranscriptomeAssemblyTools/FilterUncorrectabledPEfastq.py"
...
out_path_step02S2_FUPER="HYDRA_step02S2_filter-uncorrectable-pair-end-reads_FUPER_${thislogdate}"

## Create output directories
mkdir -p ${out_path_step02S2_FUPER}
...
date
echo "## Executing bash ipda_HYDRA_step02S2-to-pbs.sh"
...
echo
echo "## Output files saved to:       ${out_path_step02S2_FUPER}"
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "echo \"${fuper}\"" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; f1="${path_file}_R1.cor.fq*"; f2="${path_file}_R2.cor.fq*"; echo "python ${fuper} -1 ${f1} -2 ${f2} -s ${file}" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; file_path=`echo "$(dirname ${path_file})"` ; echo "mv ${file_path}/unfixrm_${file}_R1.cor.fq ${out_path_step02S2_FUPER}/" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; file_path=`echo "$(dirname ${path_file})"` ; echo "mv ${file_path}/unfixrm_${file}_R2.cor.fq ${out_path_step02S2_FUPER}/" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
## Submit PBS jobs 
ls ${pbs_stem}_*${thislogdate}.pbs | while read pbs; do echo ; echo "#................................................" ; echo "# This is PBS: ${pbs}" ;  echo "#" ; echo "# main command line(s): $(tail -n6 ${pbs} | head -n1)" ; echo "#" ; echo "# now submitting PBS" ; echo "qsub ${pbs}" ; qsub ${pbs} ; echo "#................................................" ; done

date ## Status of all user jobs (including HYDRA step 02S2 jobs) at
qstat -u "$user"
...
sed -i 's,${fuper},'"${fuper}"',g' "$logfile"
sed -i 's,${out_path_step02S2_FUPER},'"${out_path_step02S2_FUPER}"',g' "$logfile"
