...
Description: Write and submit PBS jobs for Step 03L2 (long-reads) of the
HYDRA pipeline (HYbrid De novo RNA Assembly pipeline).

Usage: bash ipda_HYDRA_step03L2-to-pbs.sh -i "path/to/input/files" -p "PBS stem" -e "email" -m INT -c INT -w "HH:MM:SS"

Resources used for pipeline development: -m 5 -c 1 -w "04:00:00"
...
-i <path/to/input/files>    Input TSV files including path FROM working
                            ...
                            Col1:
                            same line space separated files
                            path/from/working/dir/to/HYDRA_step02L3_polyA-removal_Cutadapt_DATE/stem.fastq.gz
                            
                            Col2:
                            space separated names to be used in the plots
                            for each of the same line given files

-p <PBS stem>               Stem for PBS file name
...
do
    case "${flag}" in
        ...
        ?) echo script usage: bash ipda_HYDRA_step03L2-to-pbs.sh -i path/to/input/files -p PBS stem -e email -m INT -c INT -w "HH:MM:SS" >&2
           exit;;
    esac
done
...
logfile=logfile_ipda_HYDRA_step03L2-to-pbs_${thislogdate}.txt
...
out_path_step03L2_NanoComp="HYDRA_step03L2_quality-check-after-trim_NanoComp_${thislogdate}"

## Create output directories
mkdir -p ${out_path_step03L2_NanoComp}
...
date
echo "## Executing bash ipda_HYDRA_step03L2-to-pbs.sh"
...
echo
echo "## Output files saved to:       ${out_path_step03L2_NanoComp}"
...
echo "module load ${module_python3}" >> ${pbs_stem}_${thislogdate}.pbs
echo "" >> ${pbs_stem}_${thislogdate}.pbs
...
echo "#................................................" >> ${pbs_stem}_${thislogdate}.pbs
echo "" >> ${pbs_stem}_${thislogdate}.pbs
echo 'echo "## Run NanoComp at" ; date ; echo' >> ${pbs_stem}_${thislogdate}.pbs
fastq_files=`cut -f1 ${input}`; names=`cut -f2 ${input}`
echo "NanoComp -t ${ncpus} --fastq ${fastq_files} --names ${names} -o ${out_path_step03L2_NanoComp}" >> ${pbs_stem}_${thislogdate}.pbs
...
date ## Status of all user jobs (including HYDRA step 03L2 jobs) at
qstat -u "$user"
...
sed -i 's,${out_path_step03L2_NanoComp},'"${out_path_step03L2_NanoComp}"',g' "$logfile"
