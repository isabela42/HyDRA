...
-i <path/to/input/files>    Input TSV files including path FROM working
                            ...
                            Col1:
                            path/from/working/dir/to/HYDRA_step02L3_polyA-removal_Cutadapt_DATE/stem
...
logfile=logfile_ipda_HYDRA_step03L5-to-pbs_${thislogdate}.txt
...
out_path_step03L5_FastaSplitter="HYDRA_step03L5_size-split_FastaSplitter_${thislogdate}"
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "python ${fasta_splitter} -f ${out_path_step03L5_FastaSplitter}/${file}.fasta  -n 20000 -p ${out_path_step03L5_FastaSplitter}/${file}" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
