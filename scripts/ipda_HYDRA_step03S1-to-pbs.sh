...
Description: Write and submit PBS jobs for Step 03S1 (short-reads) of the
HYDRA pipeline (HYbrid De novo RNA Assembly pipeline).

Usage: bash ipda_HYDRA_step03S1-to-pbs.sh -i "path/to/input/files" -p "PBS stem" -e "email" -m INT -c INT -w "HH:MM:SS"
...
-i <path/to/input/files>    Input TSV files including path FROM working
                            ...
                            Col1:
                            path/from/working/dir/to/HYDRA_step02S2_filter-uncorrectable-pair-end-reads_FUPER_DATE/stem
                            of both _R1.cor.fq and _R2.cor.fq files in individual lines
                            and no full stops.

                            Col2:
                            path/from/working/dir/to/HYDRA_step02S2_filter-uncorrectable-pair-end-reads_FUPER_DATE/stem_R1.cor.fq (line 1)
                            path/from/working/dir/to/HYDRA_step02S2_filter-uncorrectable-pair-end-reads_FUPER_DATE/stem_R2.cor.fq (line 2)
...
do
    case "${flag}" in
        ...
        ?) echo script usage: bash ipda_HYDRA_step03S1-to-pbs.sh -i path/to/input/files -p PBS stem -e email -m INT -c INT -w "HH:MM:SS" >&2
           exit;;
    esac
done
...
logfile=logfile_ipda_HYDRA_step03S1-to-pbs_${thislogdate}.txt
...
out_path_step03S1_FastQC="HYDRA_step03S1_quality-check-after-correction_FastQC_${thislogdate}"

## Create output directories
mkdir -p ${out_path_step03S1_FastQC}
...
date
echo "## Executing bash ipda_HYDRA_step03S1-to-pbs.sh"
...
echo
echo "## Output files saved to:       ${out_path_step03S1_FastQC}"
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; f1="${path_file}_R1.cor.fq"; f2="${path_file}_R2.cor.fq"; echo "fastqc -t ${ncpus} --outdir ${out_path_step03S1_FastQC} ${f1} ${f2} --memory 10000" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
date ## Status of all user jobs (including HYDRA step 03S1 jobs) at
qstat -u "$user"
...
sed -i 's,${out_path_step03S1_FastQC},'"${out_path_step03S1_FastQC}"',g' "$logfile"
