...
Usage: bash ipda_HYDRA_step04S1-to-pbs.sh -i "path/to/input/files" -p "PBS stem" -e "email" -m INT -c INT -w "HH:MM:SS"

Resources used for pipeline development: -m 1 -c 6 -w "02:00:00"
...
-i <path/to/input/files>    Input TSV files including path FROM working
                            ...
                            of both _R1.cor.fq and _R2.cor.fq files
                            and no full stops.
...
do
    case "${flag}" in
        ...
        ?) echo script usage: bash ipda_HYDRA_step04S1-to-pbs.sh -i path/to/input/files -p PBS stem -e email -m INT -c INT -w "HH:MM:SS" >&2
           exit;;
    esac
done
...
logfile=logfile_ipda_HYDRA_step04S1-to-pbs_${thislogdate}.txt
...
out_path_step04S1_Trimmomatic="HYDRA_step04S1_trim-reads-adapters_Trimmomatic_${thislogdate}"

## Create output directories
mkdir -p ${out_path_step04S1_Trimmomatic}
...
date
echo "## Executing bash ipda_HYDRA_step04S1-to-pbs.sh"
...
echo
echo "## Output files saved to:       ${out_path_step04S1_Trimmomatic}"
...
date ## Status of all user jobs (including HYDRA step 04S1 jobs) at
qstat -u "$user"
...
sed -i 's,${out_path_step04S1_Trimmomatic},'"${out_path_step04S1_Trimmomatic}"',g' "$logfile"
