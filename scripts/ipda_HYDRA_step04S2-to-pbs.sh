...
Description: Write and submit PBS jobs for Step 04S2 (short-reads) of the
HYDRA pipeline (HYbrid De novo RNA Assembly pipeline).

Usage: bash ipda_HYDRA_step04S2-to-pbs.sh -i "path/to/input/files" -p "PBS stem" -e "email" -m INT -c INT -w "HH:MM:SS"

Resources used for pipeline development: -m 5 -c 15 -w "00:30:00"
...
-i <path/to/input/files>    Input TSV files including path FROM working
                            ...
                            Col1:
                            path/from/working/dir/to/HYDRA_step04S1_trim-reads-adapters_Trimmomatic_DATE/stem
                            of both _R1.fq.gz and _R2.fq.gz files in individual lines
                            and no full stops.

                            Col2:
                            path/from/working/dir/to/HYDRA_step04S1_trim-reads-adapters_Trimmomatic_DATE/adapter-trimmed_*_R1.fq.gz (line 1)
                            path/from/working/dir/to/HYDRA_step04S1_trim-reads-adapters_Trimmomatic_DATE/adapter-trimmed_*_R2.fq.gz (line 2)
...
do
    case "${flag}" in
        ...
        ?) echo script usage: bash ipda_HYDRA_step04S2-to-pbs.sh -i path/to/input/files -p PBS stem -e email -m INT -c INT -w "HH:MM:SS" >&2
           exit;;
    esac
done
...
logfile=logfile_ipda_HYDRA_step04S2-to-pbs_${thislogdate}.txt
...
out_path_step04S2_BBDuk="HYDRA_step04S2_check-reads-quality-ends-avg_BBDuk_${thislogdate}"

## Create output directories
mkdir -p ${out_path_step04S2_BBDuk}
...
date
echo "## Executing bash ipda_HYDRA_step04S2-to-pbs.sh"
...
echo
echo "## Output files saved to:       ${out_path_step04S2_BBDuk}"
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; f1="${path_file}_R1.fq.gz"; f2="${path_file}_R2.fq.gz"; echo "${bbduk} t=${ncpus} -Xmx${mem}g in=${f1} in2=${f2} qtrim=rl trimq=30 minavgquality=20 minbasequality=10 minlen=75 outm=${out_path_step04S2_BBDuk}/quality-failed_${file}_R1.cor.fq.gz outm2=${out_path_step04S2_BBDuk}/quality-failed_${file}_R2.cor.fq.gz bhist=${out_path_step04S2_BBDuk}/${file}_bhist.txt qhist=${out_path_step04S2_BBDuk}/quality-check_${file}_qhist.txt gchist=${out_path_step04S2_BBDuk}/quality-check_${file}_gchist.txt aqhist=${out_path_step04S2_BBDuk}/quality-check_${file}_aqhist.txt lhist=${out_path_step04S2_BBDuk}/quality-check_${file}_lhist.txt gcbins=auto" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
date ## Status of all user jobs (including HYDRA step 04S2 jobs) at
qstat -u "$user"
...
sed -i 's,${out_path_step04S2_BBDuk},'"${out_path_step04S2_BBDuk}"',g' "$logfile"
