...
-i <path/to/input/files>    Input TSV files including path FROM working
                            ...
                            Col1:
                            path/from/working/dir/to/HYDRA_step04L3_reads-correction_FMLRC2_DATE/stem
                            of .fa.gz file and no full stops.

                            Col2:
                            path/from/working/dir/to/HYDRA_step04L3_reads-correction_FMLRC2_DATE/stem.fa.gz
...
logfile=logfile_ipda_HYDRA_step05L1-to-pbs_${thislogdate}.txt
...
# Fasta splitter
fasta_splitter="/working/lab_julietF/isabelaA/scripts/python/fasta_splitter.py"
...
out_path_step05L1_FastaSplitter="HYDRA_step05L1_size-split_FastaSplitter_${thislogdate}"
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "echo \"${fasta_splitter}\"" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "python ${fasta_splitter} -f ${out_path_step05L1_FastaSplitter}/${file}.fasta  -n 20000 -p ${out_path_step05L1_FastaSplitter}/${file}" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
sed -i 's,${fasta_splitter},'"${fasta_splitter}"',g' "$logfile"
