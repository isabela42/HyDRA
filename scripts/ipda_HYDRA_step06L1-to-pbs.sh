...
Description: Write and submit PBS jobs for Step 06L1 (short-reads) of the
HYDRA pipeline (HYbrid De novo RNA Assembly pipeline).

Usage: bash ipda_HYDRA_step06L1-to-pbs.sh -i "path/to/input/files" -p "PBS stem" -e "email" -m INT -c INT -w "HH:MM:SS"

Resources used for pipeline development: -m 5 -c 5 -w "00:30:00"
...
do
    case "${flag}" in
        ...
        ?) echo script usage: bash ipda_HYDRA_step06L1-to-pbs.sh -i path/to/input/files -p PBS stem -e email -m INT -c INT -w "HH:MM:SS" >&2
           exit;;
    esac
done
...
logfile=logfile_ipda_HYDRA_step06L1-to-pbs_${thislogdate}.txt
...
# Because of it, we only ask for 60% of the memory and CPU
...
out_path_step06L1_BBDuk="HYDRA_step06L1_ribodepletion_BBDuk_${thislogdate}"

## Create output directories
mkdir -p ${out_path_step06L1_BBDuk}
...
date
echo "## Executing bash ipda_HYDRA_step06L1-to-pbs.sh"
...
echo
echo "## Output files saved to:       ${out_path_step06L1_BBDuk}"
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "${bbduk} t=${cpuless} -Xmx${memless}g in=${path_file}.fa.gz outm=${out_path_step06L1_BBDuk}/ribosomal-RNA_${file}.fasta out=${out_path_step06L1_BBDuk}/ribo-depleted_${file}.fasta ref=${ribosomal_rna_ref} stats=${out_path_step06L1_BBDuk}/rRNA-alignment_stats.txt" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
date ## Status of all user jobs (including HYDRA step 06L1 jobs) at
qstat -u "$user"
...
sed -i 's,${out_path_step06L1_BBDuk},'"${out_path_step06L1_BBDuk}"',g' "$logfile"
