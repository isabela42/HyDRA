...
Description: Write and submit PBS jobs for Step 06S1 (short-reads) of the
HYDRA pipeline (HYbrid De novo RNA Assembly pipeline).

Usage: bash ipda_HYDRA_step06S1-to-pbs.sh -i "path/to/input/files" -p "PBS stem" -e "email" -m INT -c INT -w "HH:MM:SS"

Resources used for pipeline development: -m 2 -c 10 -w "08:00:00"
...
-i <path/to/input/files>    Input TSV files including path FROM working
                            ...
                            Col1:
                            path/from/working/dir/to/HYDRA_step04S1_trim-reads-adapters_Trimmomatic_DATE/adapter-trimmed_unfixrm_stem
                            of both _R1.fq.gz and _R2.fq.gz files in individual lines
                            and no full stops.

                            Col2:
                            path/from/working/dir/to/HYDRA_step04S1_trim-reads-adapters_Trimmomatic_DATE/adapter-trimmed_unfixrm_stem_R1.fq.gz (line 1)
                            path/from/working/dir/to/HYDRA_step04S1_trim-reads-adapters_Trimmomatic_DATE/adapter-trimmed_unfixrm_stem_R2.fq.gz (line 2)
...
do
    case "${flag}" in
        ...
        ?) echo script usage: bash ipda_HYDRA_step06S1-to-pbs.sh -i path/to/input/files -p PBS stem -e email -m INT -c INT -w "HH:MM:SS" >&2
           exit;;
    esac
done
...
logfile=logfile_ipda_HYDRA_step06S1-to-pbs_${thislogdate}.txt
...
# /working/lab_julietF/mainaB/ReferenceGenomes/Combined_human_ribosomal.fa
ribosomal_rna_ref="/working/lab_julietF/isabelaA/data/references/Combined_human_ribosomal.fa"

# Reason for fixing headers at the end of this step:
# Trimmomatic seems to change the naming scheme of reads.
...
# WARNING!
# Single End reads that survive trimming without a pair
# are discarded in the current pipeline version.

# WARNING!
# BBDuk uses more memory and CPUs than what is required.
# Because of it, we only ask for 80% of the memory and CPU
# specified when running this script, so that PBS jobs
# do not go over the required resources, as defined below:
memless=$((mem*60/100))
cpuless=$((ncpus*60/100))
...
bbduk="/working/lab_julietF/isabelaA/tools/bbmap/bbduk.sh"
...
out_path_step06S1_BBDuk="HYDRA_step06S1_ribodepletion_BBDuk_${thislogdate}"

## Create output directories
mkdir -p ${out_path_step06S1_BBDuk}
...
date
echo "## Executing bash ipda_HYDRA_step06S1-to-pbs.sh"
...
echo
echo "## Output files saved to:       ${out_path_step06S1_BBDuk}"
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo 'echo "# No HPC modules required"' >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "echo \"${bbduk}\"" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "#................................................" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo 'echo "## Run BBDuk at" ; date ; echo' >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; f1="${path_file}_R1.fq.gz"; f2="${path_file}_R2.fq.gz"; echo "${bbduk} t=${cpuless} -Xmx${memless}g in=${f1} in2=${f2} outm=${out_path_step06S1_BBDuk}/ribosomal-RNA_${file}_R1.cor.fq.gz outm2=${out_path_step06S1_BBDuk}/ribosomal-RNA_${file}_R2.cor.fq.gz out=${out_path_step06S1_BBDuk}/ribo-depleted_${file}_R1.cor.fq out2=${out_path_step06S1_BBDuk}/ribo-depleted_${file}_R2.cor.fq ref=${ribosomal_rna_ref} stats=${out_path_step06S1_BBDuk}/rRNA-alignment_stats.txt" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done

cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo 'echo "## Fix read headers at" ; date ; echo' >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; f1="${out_path_step06S1_BBDuk}/ribo-depleted_${file}_R1.cor.fq"; echo "cat ${f1} | awk '{ if (NR%4==1) { print"' $1substr($2,2)"/1" }'" else { print } }' | sed 's/_1://g' >> ${out_path_step06S1_BBDuk}/header-fixed_ribo-depleted_${file}_R1.cor.fq; gzip ${out_path_step06S1_BBDuk}/header-fixed_ribo-depleted_${file}_R1.cor.fq; gzip ${f1}" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; f2="${out_path_step06S1_BBDuk}/ribo-depleted_${file}_R2.cor.fq"; echo "cat ${f2} | awk '{ if (NR%4==1) { print"' $1substr($2,2)"/2" }'" else { print } }' | sed 's/_2://g' >> ${out_path_step06S1_BBDuk}/header-fixed_ribo-depleted_${file}_R2.cor.fq; gzip ${out_path_step06S1_BBDuk}/header-fixed_ribo-depleted_${file}_R2.cor.fq; gzip ${f2}" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
## Submit PBS jobs 
ls ${pbs_stem}_*${thislogdate}.pbs | while read pbs; do echo ; echo "#................................................" ; echo "# This is PBS: ${pbs}" ;  echo "#" ; echo "# main command line(s): $(tail -n5 ${pbs} | head -n1)" ; echo "#                       $(tail -n2 ${pbs} | head -n1)" ; echo "#                       $(tail -n1 ${pbs})" ; echo "#" ; echo "# now submitting PBS" ; echo "qsub ${pbs}" ; qsub ${pbs} ; echo "#................................................" ; done

date ## Status of all user jobs (including HYDRA step 06S1 jobs) at
qstat -u "$user"
...
sed -i 's,${out_path_step06S1_BBDuk},'"${out_path_step06S1_BBDuk}"',g' "$logfile"
