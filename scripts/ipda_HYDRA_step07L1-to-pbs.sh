...
logfile=logfile_ipda_HYDRA_step07L1-to-pbs_${thislogdate}.txt
...
out_path_step07L1_Bowtie2="HYDRA_step07L1_genome-alignment_Bowtie2_${thislogdate}"
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "bowtie2 --fast --no-unal -p${ncpus} -x ${genome_index} -f ${path_file}.fasta | samtools view -@${ncpus} -bS - >> ${out_path_step07L1_Bowtie2}/genome-alignment_${file}.bam" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
