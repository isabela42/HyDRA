...
Resources used for pipeline development: -m 150 -c 15 -w "02:00:00"
...
logfile=logfile_ipda_HYDRA_step07S1-to-pbs_${thislogdate}.txt

#................................................
#  Additional information
#................................................

# Bowtie2 index of GRCh38p7 genome:
# originally copied from
# /working/lab_julietF/mainaB/ReferenceGenomes/GenomeGRCh38p7_bowtie2
genome_index="/working/lab_julietF/isabelaA/data/references/GenomeGRCh38p7_bowtie2"
...
out_path_step07S1_Bowtie2="HYDRA_step07S1_genome-alignment_Bowtie2_${thislogdate}"
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; f1="${path_file}_R1.cor.fq.gz"; f2="${path_file}_R2.cor.fq.gz"; echo "bowtie2 --fast --no-unal -p ${ncpus} -x ${genome_index} -1 ${f1} -2 ${f2} | samtools view -@${ncpus} -bS - >> ${out_path_step07S1_Bowtie2}/genome-alignment_${file}.bam" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
sed -i 's,${genome_index},'"${genome_index}"',g' "$logfile"
