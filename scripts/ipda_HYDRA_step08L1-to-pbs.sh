...
-i <path/to/input/files>    Input TSV files including path FROM working
                            ...
                            Col1:
                            path/from/working/dir/to/HYDRA_step07L1_genome-alignment_Bowtie2_DATE/stem
                            of coordsorted.bam files and no full stops.

                            Col2:
                            path/from/working/dir/to/HYDRA_step07L1_genome-alignment_Bowtie2_DATE/stem_coordsorted.bam
...
logfile=logfile_ipda_HYDRA_step08L1-to-pbs_${thislogdate}.txt
...
# /working/lab_julietF/mainaB/ReferenceGenomes/hg38RefGene.bed & hg19RefGene.bed
#reference_genome="/working/lab_julietF/isabelaA/data/reference/hg19RefGene.bed"
reference_genome="/working/lab_julietF/isabelaA/data/references/hg38RefGene.bed"
...
out_path_step08L1_RSeQC="HYDRA_step08L1_strandness-assessment_RSeQC_${thislogdate}"
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "infer_experiment.py -r ${reference_genome} -i ${path_file}.coordsorted.bam >> ${out_path_step08L1_RSeQC}/strand-assessment_${file}.txt" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
sed -i 's,${reference_genome},'"${reference_genome}"',g' "$logfile"
