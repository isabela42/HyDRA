...
Description: Write and submit PBS jobs for Step 08S1 (short-reads) of the
HYDRA pipeline (HYbrid De novo RNA Assembly pipeline).

Usage: bash ipda_HYDRA_step08S1-to-pbs.sh -i "path/to/input/files" -p "PBS stem" -e "email" -m INT -c INT -w "HH:MM:SS"
...
-i <path/to/input/files>    Input TSV files including path FROM working
                            ...
                            Col1:
                            path/from/working/dir/to/HYDRA_step07S1_genome-alignment_Bowtie2_DATE/stem
                            of coordsorted.bam files and no full stops.

                            Col2:
                            path/from/working/dir/to/HYDRA_step07S1_genome-alignment_Bowtie2_DATE/stem_coordsorted.bam
...
do
    case "${flag}" in
        ...
        ?) echo script usage: bash ipda_HYDRA_step08S1-to-pbs.sh -i path/to/input/files -p PBS stem -e email -m INT -c INT -w "HH:MM:SS" >&2
           exit;;
    esac
done
...
logfile=logfile_ipda_HYDRA_step08S1-to-pbs_${thislogdate}.txt
...
out_path_step08S1_RSeQC="HYDRA_step08S1_strandness-assessment_RSeQC_${thislogdate}"

## Create output directories
mkdir -p ${out_path_step08S1_RSeQC}
...
date
echo "## Executing bash ipda_HYDRA_step08S1-to-pbs.sh"
...
echo
echo "## Output files saved to:       ${out_path_step08S1_RSeQC}"
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "infer_experiment.py -r ${reference_genome} -i ${path_file}.coordsorted.bam >> ${out_path_step08S1_RSeQC}/strand-assessment_${file}.txt" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
date ## Status of all user jobs (including HYDRA step 08S1 jobs) at
qstat -u "$user"
...
sed -i 's,${out_path_step08S1_RSeQC},'"${out_path_step08S1_RSeQC}"',g' "$logfile"
