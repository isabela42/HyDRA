...
logfile=logfile_ipda_HYDRA_step09L1-to-pbs_${thislogdate}.txt
...
out_path_step09L1_FastaSplitter="HYDRA_step09L1_size-split_FastaSplitter_${thislogdate}"
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "python ${fasta_splitter} -f ${path_file}.fasta  -n 20000 -p ${out_path_step09L1_FastaSplitter}/${file}" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
