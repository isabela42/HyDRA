...
Description: Write and submit PBS jobs for Step 09S2 (short-reads) of the
HYDRA pipeline (HYbrid De novo RNA Assembly pipeline).

Usage: bash ipda_HYDRA_step09S2-to-pbs.sh -i "path/to/input/folder" -p "PBS stem" -e "email" -m INT -c INT -w "HH:MM:SS"

Resources used for pipeline development: -m 1 -c 1 -w "00:30:00"

## Input:

-i <path/to/input/folder>   path/from/working/dir/to/HYDRA_step09S1_quality-check-after-ribodepletion_FastQC_DATE/
                            This folder should contain the HTML FastQC from step09S1 files.
...
do
    case "${flag}" in
        ...
        ?) echo script usage: bash ipda_HYDRA_step09S2-to-pbs.sh -i path/to/input/folder -p PBS stem -e email -m INT -c INT -w "HH:MM:SS" >&2
           exit;;
    esac
done
...
logfile=logfile_ipda_HYDRA_step09S2-to-pbs_${thislogdate}.txt
...
out_path_step09S2_MultiQC="HYDRA_step09S2_quality-check-after-ribodepletion_MultiQC_${thislogdate}"

## Create output directories
mkdir -p ${out_path_step09S2_MultiQC}
...
date
echo "## Executing bash ipda_HYDRA_step09S2-to-pbs.sh"
...
echo
echo "## Output files saved to:       ${out_path_step09S2_MultiQC}"
...
echo "${multiqc} ${input} -o ${out_path_step09S2_MultiQC}" >> ${pbs_stem}_${thislogdate}.pbs
...
date ## Status of all user jobs (including HYDRA step 09S2 jobs) at
qstat -u "$user"
...
sed -i 's,${out_path_step09S2_MultiQC},'"${out_path_step09S2_MultiQC}"',g' "$logfile"
