...
Description: Write and submit PBS jobs for Step 11H2 (hybrid) of the
HYDRA pipeline (HYbrid De novo RNA Assembly pipeline).

Usage: bash ipda_HYDRA_step11H2-to-pbs.sh -i "path/to/input/files" -p "PBS stem" -e "email" -m INT -c INT -w "HH:MM:SS"

Resources used for pipeline development: -m 250 -c 12 -w "85:00:00"
...
-i <path/to/input/files>    Input TSV files including path FROM working
                            ...
                            Col1:
                            path/from/working/dir/to/HYDRA_step06S1_ribodepletion_BBDuk_DATE/stem
                            of both _R1.cor.fq.gz and _R2.cor.fq.gz header-fixed files in individual lines
                            and no full stops.

                            Col2:
                            path/from/working/dir/to/HYDRA_step06S1_ribodepletion_BBDuk_DATE/stem_R1.cor.fq.gz (line 1)
                            path/from/working/dir/to/HYDRA_step06S1_ribodepletion_BBDuk_DATE/stem_R2.cor.fq.gz (line 2)

                            Col3:
                            String name for the de novo transcriptome assembly.
                            This string can appear multiple times on the file,
                            but only one string name is accepted.
...
do
    case "${flag}" in
        ...
        ?) echo script usage: bash ipda_HYDRA_step11H2-to-pbs.sh -i path/to/input/files -p PBS stem -e email -m INT -c INT -w "HH:MM:SS" >&2
           exit;;
    esac
done
...
logfile=logfile_ipda_HYDRA_step11H2-to-pbs_${thislogdate}.txt
...
## Disclosure:
# The first module of this pipeline and most subsequent quality assessments
# were derived from the sources below.
# Adjustments were made and additional sources consulted,
# which are mentioned at individual steps.
# <https://informatics.fas.harvard.edu/best-practices-for-de-novo-transcriptome-assembly-with-trinity.html>
# <https://github.com/trinityrnaseq/trinityrnaseq/wiki/Transcriptome-Assembly-Quality-Assessment>

## For directional libraries,
# --SS_lib_type should be set to FR or RF for ligation-stranded
# and dUTP-based library construction, respectively.
# The PE fragments are strand-specific,
# with left end on the Reverse strand and the right end on Forward
# strand of the sequenced mRNA template.
# One can get more information here:
# <https://databeauty.com/blog/opinion/2016/09/21/RNA-seq-strand-issues.html>

## The parameters above are designed to prevent fragmented transcripts.
# Especifically, keeping --min_kmer_cov at default, and
# setting --normalize_max_read_cov to 50.
# Raising --min_kmer_cov to 2 (default is 1)
# can help to eliminate error-containing kmers, even for deeply sequenced samples.
# Running TransRate and Detonate will also indicate
# which assembly is best if different parameters are tested.

## Possible run problems (and solutions):
# <https://github.com/trinityrnaseq/trinityrnaseq/issues/1048>
# <https://groups.google.com/g/trinityrnaseq-users/c/jhWTmLcmWfk>
# <https://groups.google.com/g/trinityrnaseq-users/c/1OuDRTPv8eo/m/7xY8OyJiEAAJ>
...
## Accodring to Trinity:
## "output directory must contain the word 'trinity' as a safety precaution,
## given that auto-deletion can take place.""
out_path_step11H2_Trinity="HYDRA_step11H2_de-novo-transcriptome-assembly_Trinity_${thislogdate}"

## Create output directories
mkdir -p ${out_path_step11H2_Trinity}
...
date
echo "## Executing bash ipda_HYDRA_step11H2-to-pbs.sh"
...
echo
echo "## Output files saved to:       ${out_path_step11H2_Trinity}"
...
echo "module load ${module_trinity}" >> ${pbs_stem}_${thislogdate}.pbs
echo "module load ${module_samtools}" >> ${pbs_stem}_${thislogdate}.pbs
echo "" >> ${pbs_stem}_${thislogdate}.pbs
echo 'echo "## Path to user-installed tools"' >> ${pbs_stem}_${thislogdate}.pbs
echo 'echo "# None required"' >> ${pbs_stem}_${thislogdate}.pbs
echo "" >> ${pbs_stem}_${thislogdate}.pbs
...
echo "#................................................" >> ${pbs_stem}_${thislogdate}.pbs
echo "" >> ${pbs_stem}_${thislogdate}.pbs
echo 'echo "## Run Trinity at" ; date ; echo' >> ${pbs_stem}_${thislogdate}.pbs
cut -f1 ${input} | sort | uniq | while read path_file; do f1="${path_file}_R1.cor.fq.gz"; f2="${path_file}_R2.cor.fq.gz"; ls ${f1} >> ${pbs_stem}_trinity-left-list_${thislogdate}.txt ; ls ${f2} >> ${pbs_stem}_trinity-right-list_${thislogdate}.txt ; done ; left_list=`cat ${pbs_stem}_trinity-left-list_${thislogdate}.txt | tr '\n' ',' | tr -d ' '`; left_list="${left_list%\,}"; right_list=`cat ${pbs_stem}_trinity-right-list_${thislogdate}.txt | tr '\n' ',' | tr -d ' '`; right_list="${right_list%\,}"
echo "Trinity --seqType fq --max_memory ${mem}G --left $left_list --right $right_list --CPU ${ncpus} --SS_lib_type RF --output ${out_path_step11H2_Trinity} --group_pairs_distance 280 --full_cleanup --bflyHeapSpaceMax 16G --normalize_max_read_cov 50 --bypass_java_version_check" >> ${pbs_stem}_${thislogdate}.pbs
#echo "Trinity --seqType fq --max_memory ${mem}G --left $left_list --right $right_list --CPU ${ncpus} --SS_lib_type RF --output ${out_path_step11H2_Trinity} --group_pairs_distance 280 --full_cleanup --bflyHeapSpaceMax 16G --normalize_max_read_cov 50 --bypass_java_version_check --FORCE" >> ${pbs_stem}_${thislogdate}.pbs
echo "" >> ${pbs_stem}_${thislogdate}.pbs

echo 'echo "## Move trinity files to output folder at" ; date ; echo' >> ${pbs_stem}_${thislogdate}.pbs
echo "mv ${pbs_stem}_trinity-left-list_${thislogdate}.txt ${out_path_step11H2_Trinity}" >> ${pbs_stem}_${thislogdate}.pbs
echo "mv ${pbs_stem}_trinity-right-list_${thislogdate}.txt ${out_path_step11H2_Trinity}" >> ${pbs_stem}_${thislogdate}.pbs
assembly=`cut -f3 ${input} | sort | uniq` ; echo "mv ${out_path_step11H2_Trinity}.Trinity.fasta ${out_path_step11H2_Trinity}/${assembly}.Trinity.fasta" >> ${pbs_stem}_${thislogdate}.pbs
assembly=`cut -f3 ${input} | sort | uniq` ; echo "mv ${out_path_step11H2_Trinity}.Trinity.fasta.gene_trans_map ${out_path_step11H2_Trinity}/${assembly}.Trinity.fasta.gene_trans_map" >> ${pbs_stem}_${thislogdate}.pbs
...
## Submit PBS jobs 
ls ${pbs_stem}_*${thislogdate}.pbs | while read pbs; do echo ; echo "#................................................" ; echo "# This is PBS: ${pbs}" ;  echo "#" ; echo "# main command line(s): $(tail -n7 ${pbs} | head -n1)" ; echo "#                       $(tail -n4 ${pbs} | head -n1)" ; echo "#                       $(tail -n3 ${pbs} | head -n1)" ; echo "#                       $(tail -n2 ${pbs} | head -n1)" ; echo "#                       $(tail -n1 ${pbs})" ; echo "#" ; echo "# now submitting PBS" ; echo "qsub ${pbs}" ; qsub ${pbs} ; echo "#................................................" ; done

date ## Status of all user jobs (including HYDRA step 11H2 jobs) at
qstat -u "$user"
...
sed -i 's,${out_path_step11H2_Trinity},'"${out_path_step11H2_Trinity}"',g' "$logfile"
