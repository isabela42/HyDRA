...
Description: Write and submit PBS jobs for Step 12H3 (short-reads based
transcriptome) of the HYDRA pipeline (HYbrid De novo RNA Assembly pipeline).

Usage: bash ipda_HYDRA_step12H3-to-pbs.sh -i "path/to/input/files" -p "PBS stem" -e "email" -m INT -c INT -w "HH:MM:SS"

Resources used for pipeline development: -m 80 -c 15 -w "120:00:00"
...
do
    case "${flag}" in
        ...
        ?) echo script usage: bash ipda_HYDRA_step12H3-to-pbs.sh -i path/to/input/files -p PBS stem -e email -m INT -c INT -w "HH:MM:SS" >&2
           exit;;
    esac
done
...
logfile=logfile_ipda_HYDRA_step12H3-to-pbs_${thislogdate}.txt
...
out_path_step12H3_TransRate="HYDRA_step12H3_polish-assembly_TransRate_${thislogdate}"

## Create output directories
mkdir -p ${out_path_step12H3_TransRate}
...
date
echo "## Executing bash ipda_HYDRA_step12H3-to-pbs.sh"
...
echo
echo "## Output files saved to:       ${out_path_step12H3_TransRate}"
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "transrate --assembly ${path_file} --reference ${reference_fasta} --threads ${ncpus} --output ${out_path_step12H3_TransRate}" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "mv ${out_path_step12H3_TransRate}/assemblies.csv ${out_path_step12H3_TransRate}/${file}_assemblies.csv" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
date ## Status of all user jobs (including HYDRA step 12H3 jobs) at
qstat -u "$user"
...
sed -i 's,${out_path_step12H3_TransRate},'"${out_path_step12H3_TransRate}"',g' "$logfile"
