...
-i <path/to/input/files>    Input TSV files including path FROM working
                            ...
                            Col2:
                            path/from/working/dir/to/HYDRA_step11H2_de-novo-transcriptome-assembly_*_DATE/insilico_read_normalization/left.norm.fq
                            of .Trinity.fasta derived files.

                            Col3:
                            path/from/working/dir/to/HYDRA_step11H2_de-novo-transcriptome-assembly_*_DATE/insilico_read_normalization/right.norm.fq
                            of .Trinity.fasta derived files.
...
reference_fasta=/working/lab_julietF/isabelaA/data/references/T2TCHM13v2/ensembl_T2T009914755.4_transcriptome.fa # T2T ensembl reference
#reference_fasta=/working/lab_julietF/mainaB/ReferenceGenomes/TranscriptomeGRCh38p7_ERCC.fa # GRCh38p7_ERCC reference
...
out_path_step12H4_TransRate="HYDRA_step12H4_polish-assembly_TransRate_${thislogdate}"
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; leftfile=`grep "${path_file}" ${input} | cut -f2`; rightfile=`grep "${path_file}" ${input} | cut -f3`; echo "transrate --assembly ${path_file} --left ${leftfile}.paired.fq --right ${rightfile}.paired.fq --reference ${reference_fasta} --threads ${ncpus} --output ${out_path_step12H4_TransRate}" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
