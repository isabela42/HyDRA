...
-i <path/to/input/files>    Input TSV files including path FROM working
                            ...
                            Col2:
                            path/from/working/dir/to/HYDRA_step18H1_transcriptome-annotation_BedTools_DATE/stem/stem.bed
...
logfile=logfile_ipda_HYDRA_step18H1-to-pbs_${thislogdate}.txt
...
# PLEK 1.2 (From ezLncPred but downloaded sepparetely)
...
out_path_step18H1_ezLncPred="HYDRA_step18H1_coding-potential_ezLncPred_${thislogdate}"
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file}" | sed 's/\(.*\)\..*/\1/')"` ; mkdir -p ${out_path_step18H1_ezLncPred}/${file} ; annotation=`grep ${path_file} ${input} | cut -f2` ; main_annotation=`echo "$(basename "${annotation}" | sed 's/\(.*\)\..*/\1/')"` ; echo "bedtools intersect -wo -s -f 0.6 -r -a ${annotation} -b ${genome_proteins} >> ${out_path_step18H1_ezLncPred}/${file}/${main_annotation}.proteincoding.bed" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file}" | sed 's/\(.*\)\..*/\1/')"` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
sed -i 's,${genome_proteins},'"${genome_proteins}"',g' "$logfile"
