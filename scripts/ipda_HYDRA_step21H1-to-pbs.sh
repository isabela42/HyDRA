...
-i <path/to/input/files>    Input TSV files including path FROM working
                            ...
                            Col1:
                            path/from/working/dir/to/HYDRA_step18H*_coding-potential_ezLncPred_DATE/stem/
                            ezlncpred-and-feelnc.noncoding

                            Col2:
                            path/from/working/dir/to/HYDRA_step19H*_lncRNA-potential_FEELnc_DATE/stem/
                            stem.gtf
...
logfile=logfile_ipda_HYDRA_step21H1-to-pbs_${thislogdate}.txt
...
out_path_step21H1_Bash="HYDRA_step21H1_metrics-lncRNAs_GeneStats_${thislogdate}"
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=$(echo "$path_file" | awk -F'/' '{print $NF}' | awk -F'.' '{for (i=1; i<NF-2; i++) printf "%s.", $i; print $(NF-2)}') ; echo "#................................................" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=$(echo "$path_file" | awk -F'/' '{print $NF}' | awk -F'.' '{for (i=1; i<NF-2; i++) printf "%s.", $i; print $(NF-2)}') ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=$(echo "$path_file" | awk -F'/' '{print $NF}' | awk -F'.' '{for (i=1; i<NF-2; i++) printf "%s.", $i; print $(NF-2)}') ; echo 'echo "## Load tools from HPC"' >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=$(echo "$path_file" | awk -F'/' '{print $NF}' | awk -F'.' '{for (i=1; i<NF-2; i++) printf "%s.", $i; print $(NF-2)}') ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=$(echo "$path_file" | awk -F'/' '{print $NF}' | awk -F'.' '{for (i=1; i<NF-2; i++) printf "%s.", $i; print $(NF-2)}') ; echo 'echo "## Path to user-installed tools"' >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=$(echo "$path_file" | awk -F'/' '{print $NF}' | awk -F'.' '{for (i=1; i<NF-2; i++) printf "%s.", $i; print $(NF-2)}') ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
