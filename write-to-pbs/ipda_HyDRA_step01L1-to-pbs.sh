...
logfile=logfile_ipda_hydra01L1-to-pbs_${thislogdate}.txt
...
out_path_step01L1_NanoPlot="hydra01L1_qc-raw_NanoPlot_${thislogdate}"
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; mkdir -p ${out_path_step01L1_NanoPlot}/${file}; f1="${path_file}.f*"; echo "NanoPlot -t ${ncpus} -o ${out_path_step01L1_NanoPlot}/${file} --fastq ${f1}" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
