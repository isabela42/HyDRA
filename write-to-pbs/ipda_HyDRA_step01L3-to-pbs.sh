...
logfile=logfile_ipda_hydra01L3-to-pbs_${thislogdate}.txt
...
out_path_step01L3_FastQC="hydra01L3_qc-raw_FastQC_${thislogdate}"
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; f1="${path_file}*f*"; echo "fastqc -t ${ncpus} --outdir ${out_path_step01L3_FastQC} ${f1} --memory 10000" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
