...
Description: Write and submit PBS jobs for Step 01L4 (long-reads) of the
HYDRA pipeline (HYbrid De novo RNA Assembly pipeline).

Usage: bash ipda_HyDRA_step01L4-to-pbs.sh -i "path/to/input/folder" -p "PBS stem" -e "email" -m INT -c INT -w "HH:MM:SS"

Resources used for pipeline development: -m 1 -c 1 -w "00:30:00"

## Input:

-i <path/to/input/folder>   path/from/working/dir/to/hydra01L3_qc-raw_FastQC_DATE/
                            This folder should contain the HTML FastQC from step01L3 files.
...
do
    case "${flag}" in
        ...
        ?) echo script usage: bash ipda_HyDRA_step01L4-to-pbs.sh -i path/to/input/folder -p PBS stem -e email -m INT -c INT -w "HH:MM:SS" >&2
           exit;;
    esac
done
...
logfile=logfile_ipda_hydra01L4-to-pbs_${thislogdate}.txt
...
out_path_step01L4_MultiQC="hydra01L4_qc-raw_MultiQC_${thislogdate}"

## Create output directories
mkdir -p ${out_path_step01L4_MultiQC}
...
date
echo "## Executing bash ipda_HyDRA_step01L4-to-pbs.sh"
...
echo
echo "## Output files saved to:       ${out_path_step01L4_MultiQC}"
...
echo "${module_multiqc} ${input} -o ${out_path_step01L4_MultiQC}" >> ${pbs_stem}_${thislogdate}.pbs
...
date ## Status of all user jobs (including HYDRA step 01L4 jobs) at
qstat -u "$user"
...
sed -i 's,${out_path_step01L4_MultiQC},'"${out_path_step01L4_MultiQC}"',g' "$logfile"
