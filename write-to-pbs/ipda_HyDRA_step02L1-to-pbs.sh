...
Description: Write and submit PBS jobs for Step 02L1 (long-reads) of the
HYDRA pipeline (HYbrid De novo RNA Assembly pipeline).

Usage: bash ipda_HyDRA_step02L1-to-pbs.sh -i "path/to/input/files" -p "PBS stem" -e "email" -m INT -c INT -w "HH:MM:SS"

Resources used for pipeline development: -m 160 -c 40 -w "24:00:00"
...
do
    case "${flag}" in
        ...
        ?) echo script usage: bash ipda_HyDRA_step02L1-to-pbs.sh -i path/to/input/files -p PBS stem -e email -m INT -c INT -w "HH:MM:SS" >&2
           exit;;
    esac
done
...
logfile=logfile_ipda_hydra02L1-to-pbs_${thislogdate}.txt
...
# Porechop v0.2.4:
# <https://github.com/rrwick/Porechop/tree/master>
# <https://academic.oup.com/bioinformaticsadvances/article/3/1/vbac085/6835406>
module_porechop=Porechop/0.2.4
...
out_path_step02L1_Porechop="hydra02L1_trim-adapters_Porechop_${thislogdate}"

## Create output directories
mkdir -p ${out_path_step02L1_Porechop}
...
date
echo "## Executing bash ipda_HyDRA_step02L1-to-pbs.sh"
...
echo
echo "## Output files saved to:       ${out_path_step02L1_Porechop}"
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "#................................................" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "module load ${module_porechop}" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "#................................................" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo 'echo "## Run Porechop at" ; date ; echo' >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "porechop -i ${path_file}*f* --threads ${ncpus} -o ${out_path_step02L1_Porechop}/adapter-trimmed_${file}.fastq.gz" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
#cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; if [ -f ${path_file}*f*.gz ]; then echo "porechop -i ${path_file}*f*.gz --threads ${ncpus} -o ${out_path_step02L1_Porechop}/adapter-trimmed_${file}.fastq.gz" >> ${pbs_stem}_${file}_${thislogdate}.pbs; else echo "ls ${path_file}*f* | while read file; do gzip -c \${file} >> \${file}.gz ; done ; porechop -i ${path_file}*f*.gz --threads ${ncpus} -o ${out_path_step02L1_Porechop}/adapter-trimmed_${file}.fastq.gz" >> ${pbs_stem}_${file}_${thislogdate}.pbs; fi; done
...
date ## Status of all user jobs (including HYDRA step 02L1 jobs) at
qstat -u "$user"
...
sed -i 's,${module_porechop},'"${module_porechop}"',g' "$logfile"
sed -i 's,${out_path_step02L1_Porechop},'"${out_path_step02L1_Porechop}"',g' "$logfile"
