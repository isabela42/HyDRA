...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; flag_merge=`grep "${path_file}" ${input} | cut -f4 | sort | uniq`; if [[ $flag_merge = "yes" ]]; then f1="${out_path_step02S1_Rcorrector}/${file}_R1.fastq.gz"; f2="${out_path_step02S1_Rcorrector}/${file}_R2.fastq.gz"; elif [[ $flag_merge = "no" ]]; then f1=`grep "${path_file}" ${input} | cut -f2`; f2=`grep "${path_file}" ${input} | cut -f3`; fi; echo "perl ${module_rcorrector} -t ${ncpus} -od ${out_path_step02S1_Rcorrector} -1 ${f1} -2 ${f2}" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "f1=\`ls ${out_path_step02S1_Rcorrector}/${file}*1.cor.f*\`; f2=\`ls ${out_path_step02S1_Rcorrector}/${file}*2.cor.f*\`; bash ${module_reformat} in=\${f1} in2=\${f2}" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
