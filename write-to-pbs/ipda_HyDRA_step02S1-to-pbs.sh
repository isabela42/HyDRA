...
-i <path/to/input/files>    Input TSV files including path FROM working
                            ...
                            Col5:
                            single|paired
                            Flag for single-end or paired-end reads. If Col5 is single, it will run with F1 only.
                            Note: please leave Col3 empty
...
logfile=logfile_ipda_hydra02S1-to-pbs_${thislogdate}.txt
...
module_rcorrector="/working/lab_julietF/isabelaA/tools/rcorrector/run_rcorrector.pl"

# Reformat (BBTools suite):
# <https://jgi.doe.gov/data-and-tools/software-tools/bbtools/bb-tools-user-guide/>
module_reformat="/working/lab_julietF/isabelaA/tools/bbmap/reformat.sh"
...
out_path_step02S1_Rcorrector="hydra02S1_short-cor_Rcorrector_${thislogdate}"
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "#................................................" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "echo \"${module_rcorrector}\"" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "echo \"${module_reformat}\"" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; flag_reads=`grep -P "${path_file}\t" ${input} | cut -f5`; if [[ $flag_reads = "single" ]]; then continue; else flag_merge=`grep "${path_file}" ${input} | cut -f4 | sort | uniq`; if [[ $flag_merge = "yes" ]]; then f2=`grep "${path_file}" ${input} | cut -f3`; echo "zcat -c ${f2} | gzip -c >> ${out_path_step02S1_Rcorrector}/${file}_R2.fastq.gz" >> ${pbs_stem}_${file}_${thislogdate}.pbs; elif [[ $flag_merge = "no" ]]; then echo "#Flag to do not merge file" >> ${pbs_stem}_${file}_${thislogdate}.pbs; else echo "Unknown flag provided. Please provide either yes or no string and try again" >> ${pbs_stem}_${file}_${thislogdate}.pbs; fi; fi; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; flag_merge=`grep "${path_file}" ${input} | cut -f4 | sort | uniq`; flag_reads=`grep -P "${path_file}\t" ${input} | cut -f5`; if [[ $flag_merge = "yes" ]]; then f1="${out_path_step02S1_Rcorrector}/${file}_R1.fastq.gz"; f2="${out_path_step02S1_Rcorrector}/${file}_R2.fastq.gz"; elif [[ $flag_merge = "no" ]]; then f1=`grep "${path_file}" ${input} | cut -f2`; f2=`grep "${path_file}" ${input} | cut -f3`; fi; if [[ $flag_reads = "single" ]]; then reads="-1 ${f1}"; else reads="-1 ${f1} -2 ${f2}"; fi; echo "perl ${module_rcorrector} -t ${ncpus} -od ${out_path_step02S1_Rcorrector} ${reads}" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
sed -i 's,${module_rcorrector},'"${module_rcorrector}"',g' "$logfile"
sed -i 's,${module_reformat},'"${module_reformat}"',g' "$logfile"
