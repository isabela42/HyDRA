...
logfile=logfile_ipda_hydra02S2-to-pbs_${thislogdate}.txt
...
# Python 2.7.13: 
# IPDA: we used 2.7.10 whn developing HyDRA
module_python=python/2.7.13

# FUPER (FilterUncorrectabledPEfastq):
# </working/lab_julietF/isabelaA/tools/TranscriptomeAssemblyTools/FilterUncorrectabledPEfastq.py> # new version uses python3, hydra was developed on the original one
module_fuper="/working/lab_julietF/isabelaA/tools/TranscriptomeAssemblyTools/FilterUncorrectabledPEfastq.py"
...
out_path_step02S2_FUPER="hydra02S2_filter-uncor_FUPER_${thislogdate}"
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "echo \"${module_fuper}\"" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
sed -i 's,${module_fuper},'"${module_fuper}"',g' "$logfile"
