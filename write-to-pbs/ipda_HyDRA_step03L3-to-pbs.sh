...
Description: Write and submit PBS jobs for Step 03L3 (long-reads) of the
HYDRA pipeline (HYbrid De novo RNA Assembly pipeline).

Usage: bash ipda_HyDRA_step03L3-to-pbs.sh -i "path/to/input/files" -p "PBS stem" -e "email" -m INT -c INT -w "HH:MM:SS"
...
do
    case "${flag}" in
        ...
        ?) echo script usage: bash ipda_HyDRA_step03L3-to-pbs.sh -i path/to/input/files -p PBS stem -e email -m INT -c INT -w "HH:MM:SS" >&2
           exit;;
    esac
done
...
logfile=logfile_ipda_hydra03L3-to-pbs_${thislogdate}.txt
...
out_path_step03L3_FastQC="hydra03L3_qc-trim_FastQC_${thislogdate}"

## Create output directories
mkdir -p ${out_path_step03L3_FastQC}
...
date
echo "## Executing bash ipda_HyDRA_step03L3-to-pbs.sh"
...
echo
echo "## Output files saved to:       ${out_path_step03L3_FastQC}"
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; f1="${path_file}.fastq.gz"; echo "fastqc -t ${ncpus} --outdir ${out_path_step03L3_FastQC} ${f1} --memory 10000" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
date ## Status of all user jobs (including HYDRA step 03L3 jobs) at
qstat -u "$user"
...
sed -i 's,${out_path_step03L3_FastQC},'"${out_path_step03L3_FastQC}"',g' "$logfile"
