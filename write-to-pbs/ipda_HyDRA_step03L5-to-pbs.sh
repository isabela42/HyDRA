...
Description: Write and submit PBS jobs for Step 03L5 (long-reads) of the
HYDRA pipeline (HYbrid De novo RNA Assembly pipeline).

Usage: bash ipda_HyDRA_step03L5-to-pbs.sh -i "path/to/input/files" -p "PBS stem" -e "email" -m INT -c INT -w "HH:MM:SS"
...
-i <path/to/input/files>    Input TSV files including path FROM working
                            ...
                            Col1:
                            path/from/working/dir/to/hydra02L3_trim-polyA_Cutadapt_DATE/polyA-trimmed_quality-trimmed_adapter-trimmed_stem
                            of .fastq.gz file and no full stops.

                            Col2:
                            path/from/working/dir/to/hydra02L3_trim-polyA_Cutadapt_DATE/polyA-trimmed_quality-trimmed_adapter-trimmed_stem.fastq.gz
...
do
    case "${flag}" in
        ...
        ?) echo script usage: bash ipda_HyDRA_step03L5-to-pbs.sh -i path/to/input/files -p PBS stem -e email -m INT -c INT -w "HH:MM:SS" >&2
           exit;;
    esac
done
...
logfile=logfile_ipda_hydra03L5-to-pbs_${thislogdate}.txt
...
out_path_step03L5_FastaSplitter="hydra03L5_size-split_FastaSplitter_${thislogdate}"

## Create output directories
mkdir -p ${out_path_step03L5_FastaSplitter}
...
date
echo "## Executing bash ipda_HyDRA_step03L5-to-pbs.sh"
...
echo
echo "## Output files saved to:       ${out_path_step03L5_FastaSplitter}"
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "#................................................" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo 'echo "## Run seqtk at" ; date ; echo' >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "seqtk seq -a ${path_file}.fastq.gz > ${out_path_step03L5_FastaSplitter}/${file}.fasta" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "python ${module_fastasplitter} -f ${out_path_step03L5_FastaSplitter}/${file}.fasta  -n 20000 -p ${out_path_step03L5_FastaSplitter}/${file}" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
date ## Status of all user jobs (including HYDRA step 03L5 jobs) at
qstat -u "$user"
...
sed -i 's,${out_path_step03L5_FastaSplitter},'"${out_path_step03L5_FastaSplitter}"',g' "$logfile"
