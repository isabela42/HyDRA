...
-i <path/to/input/files>    Input TSV files including path FROM working
                            ...
                            Col1:
                            path/from/working/dir/to/hydra02S2_filter-uncor_FUPER_DATE/unfixrm_stem
                            of both *1.cor.f* and *2.cor.f* files in individual lines
                            and no full stops.
                            Extensions accepted: .fastq.gz/fq.gz/fastq/fq

                            Col2:
                            path/from/working/dir/to/hydra02S2_filter-uncor_FUPER_DATE/unfixrm_stem_R1.cor.fq (line 1)
                            path/from/working/dir/to/hydra02S2_filter-uncor_FUPER_DATE/unfixrm_stem_R2.cor.fq (line 2)
                            Extensions accepted: .fastq.gz/fq.gz/fastq/fq
...
logfile=logfile_ipda_hydra03S1-to-pbs_${thislogdate}.txt
...
out_path_step03S1_FastQC="hydra03S1_qc-cor_FastQC_${thislogdate}"
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; f1=(${path_file}*_{R1,1}*.cor.fq); f2=(${path_file}*_{R2,2}*.cor.fq); echo "fastqc -t ${ncpus} --outdir ${out_path_step03S1_FastQC} ${f1} ${f2} --memory 10000" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
