...
-i <path/to/input/files>    Input TSV files including path FROM working
                            ...
                            ls -d path/from/working/dir/to/hydra04S1_trim-adapters_Trimmomatic_DATE/adapter-trimmed_unfixrm_*.fq.gz | tr '\n' ' '
...
logfile=logfile_ipda_hydra04L1-to-pbs_${thislogdate}.txt
...
module_ropebwt2="conda-envs/ropebwt2-r187"
...
out_path_step04L1_RopeBWT2="hydra04L1_short-reads-bwt_RopeBWT2_${thislogdate}"
...
cut -f1 ${input} | sort | uniq | while read file; do echo "#................................................" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read file; do echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read file; do echo "module load ${module_ropebwt2}" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read file; do echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
