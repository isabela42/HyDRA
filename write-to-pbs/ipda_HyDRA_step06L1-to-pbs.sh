...
WARNING: BBDuk uses more memory and CPUs than what is required. Therefore, we only ask for 60% of the memory and CPU specified when running this step.
...
logfile=logfile_ipda_hydra06L1-to-pbs_${thislogdate}.txt
...
out_path_step06L1_BBDuk="hydra06L1_ribodepletion_BBDuk_${thislogdate}"
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; ribosomal_rna_ref=`grep ${path_file} ${input} | cut -f3 | sort | uniq`; echo "${module_bbduk} t=${cpuless} -Xmx${memless}g in=${path_file}.fa.gz outm=${out_path_step06L1_BBDuk}/ribosomal-RNA_${file}.fasta out=${out_path_step06L1_BBDuk}/ribo-depleted_${file}.fasta ref=${ribosomal_rna_ref} stats=${out_path_step06L1_BBDuk}/rRNA-alignment_stats.txt" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
