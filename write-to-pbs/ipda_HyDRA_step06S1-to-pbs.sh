...
WARNING 1: Reason for fixing headers at the end of this step: Trimmomatic seems to change the naming scheme of reads. Original is "/1" for left read and "/2" for right reads respectively. Trinity needs the naming to be kept as expected. Source: https://www.biostars.org/p/141602/
WARNING 2: Single End reads that survive trimming without a pair are discarded in the current pipeline version.
...
-i <path/to/input/files>    Input TSV files including path FROM working
                            ...
                            Col1:
                            path/from/working/dir/to/hydra04S1_trim-adapters_Trimmomatic_DATE/adapter-trimmed_unfixrm_stem
                            of both _R1.fq.gz and _R2.fq.gz files in individual lines
                            and no full stops.

                            Col2:
                            path/from/working/dir/to/hydra04S1_trim-adapters_Trimmomatic_DATE/adapter-trimmed_unfixrm_stem_R1.fq.gz (line 1)
                            path/from/working/dir/to/hydra04S1_trim-adapters_Trimmomatic_DATE/adapter-trimmed_unfixrm_stem_R2.fq.gz (line 2)

                            Col3:
                            path/from/working/dir/to/rRNA.fasta
                            File to use as reference for rRNA removal.
...
logfile=logfile_ipda_hydra06S1-to-pbs_${thislogdate}.txt
...
module_bbduk="/working/lab_julietF/isabelaA/tools/bbmap/bbduk.sh"
...
out_path_step06S1_BBDuk="hydra06S1_ribodepletion_BBDuk_${thislogdate}"
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "#................................................" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "echo \"${module_bbduk}\"" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
sed -i 's,${module_bbduk},'"${module_bbduk}"',g' "$logfile"
