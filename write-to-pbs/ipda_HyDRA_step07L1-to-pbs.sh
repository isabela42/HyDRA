...
Description: Write and submit PBS jobs for Step 07L1 (long-reads) of the
HYDRA pipeline (HYbrid De novo RNA Assembly pipeline).

Usage: bash ipda_HyDRA_step07L1-to-pbs.sh -i "path/to/input/files" -p "PBS stem" -e "email" -m INT -c INT -w "HH:MM:SS"

Resources used for pipeline development: -m 150 -c 15 -w "30:00:00"
...
-i <path/to/input/files>    Input TSV files including path FROM working
                            ...
                            Col1:
                            path/from/working/dir/to/hydra06L1_ribodepletion_BBDuk_DATE/ribo-depleted_corrected_polyA-trimmed_quality-trimmed_adapter-trimmed_stem
                            of .fasta file and no full stops.

                            Col2:
                            path/from/working/dir/to/hydra06L1_ribodepletion_BBDuk_DATE/ribo-depleted_corrected_polyA-trimmed_quality-trimmed_adapter-trimmed_stem.fasta
...
do
    case "${flag}" in
        ...
        ?) echo script usage: bash ipda_HyDRA_step07L1-to-pbs.sh -i path/to/input/files -p PBS stem -e email -m INT -c INT -w "HH:MM:SS" >&2
           exit;;
    esac
done
...
logfile=logfile_ipda_hydra07L1-to-pbs_${thislogdate}.txt
...
out_path_step07L1_Bowtie2="hydra07L1_gen-map_Bowtie2_${thislogdate}"

## Create output directories
mkdir -p ${out_path_step07L1_Bowtie2}
...
date
echo "## Executing bash ipda_HyDRA_step07L1-to-pbs.sh"
...
echo
echo "## Output files saved to:       ${out_path_step07L1_Bowtie2}"
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; genome_bowtie2_index=`grep ${path_file} ${input} | cut -f3 | sort | uniq`; echo "bowtie2 --fast --no-unal -p ${ncpus} -x ${genome_bowtie2_index} -f ${path_file}.fasta | samtools view -@${ncpus} -bS - >> ${out_path_step07L1_Bowtie2}/genome-alignment_${file}.bam" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "samtools sort -@${ncpus} ${out_path_step07L1_Bowtie2}/genome-alignment_${file}.bam -o ${out_path_step07L1_Bowtie2}/genome-alignment_${file}.coordsorted.bam" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "samtools index -@${ncpus} ${out_path_step07L1_Bowtie2}/genome-alignment_${file}.coordsorted.bam ${out_path_step07L1_Bowtie2}/genome-alignment_${file}.coordsorted.bam.bai" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
date ## Status of all user jobs (including HYDRA step 07L1 jobs) at
qstat -u "$user"
...
sed -i 's,${out_path_step07L1_Bowtie2},'"${out_path_step07L1_Bowtie2}"',g' "$logfile"
