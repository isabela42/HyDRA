...
Description: Write and submit PBS jobs for Step 08L1 (short-reads) of the
HYDRA pipeline (HYbrid De novo RNA Assembly pipeline).

Usage: bash ipda_HyDRA_step08L1-to-pbs.sh -i "path/to/input/files" -p "PBS stem" -e "email" -m INT -c INT -w "HH:MM:SS"
...
-i <path/to/input/files>    Input TSV files including path FROM working
                            ...
                            Col1:
                            path/from/working/dir/to/hydra07L1_gen-map_Bowtie2_DATE/genome-alignment_ribo-depleted_corrected_polyA-trimmed_quality-trimmed_adapter-trimmed_stem
                            of coordsorted.bam files and no full stops.

                            Col2:
                            path/from/working/dir/tohydra07L1_gen-map_Bowtie2_DATE/genome-alignment_ribo-depleted_corrected_polyA-trimmed_quality-trimmed_adapter-trimmed_stem_coordsorted.bam

                            Col3:
                            path/from/working/dir/to/stemRefGene.bed
                            Note download reference gene files (e.g.:
                            ...
                            unzip files and use sed to change chromosome nomenclature (to match your reference
                            genome): sed 's/^chr//g'. WARNING! save files as stemRefGene.bed. 
...
do
    case "${flag}" in
        ...
        ?) echo script usage: bash ipda_HyDRA_step08L1-to-pbs.sh -i path/to/input/files -p PBS stem -e email -m INT -c INT -w "HH:MM:SS" >&2
           exit;;
    esac
done
...
logfile=logfile_ipda_hydra08L1-to-pbs_${thislogdate}.txt
...
out_path_step08L1_RSeQC="hydra08L1_strandness-assess_RSeQC_${thislogdate}"

## Create output directories
mkdir -p ${out_path_step08L1_RSeQC}
...
date
echo "## Executing bash ipda_HyDRA_step08L1-to-pbs.sh"
...
echo
echo "## Output files saved to:       ${out_path_step08L1_RSeQC}"
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "#................................................" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "module load ${module_rseqc}" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "#................................................" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo 'echo "## Run RSeQC at" ; date ; echo' >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; reference_gene=`grep ${path_file} ${input} | cut -f3 | sort | uniq`; echo "infer_experiment.py -r ${reference_gene} -i ${path_file}.coordsorted.bam >> ${out_path_step08L1_RSeQC}/strand-assessment_${file}.txt" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
date ## Status of all user jobs (including HYDRA step 08L1 jobs) at
qstat -u "$user"
...
sed -i 's,${module_rseqc},'"${module_rseqc}"',g' "$logfile"
sed -i 's,${reference_gene},'"${reference_gene}"',g' "$logfile"
sed -i 's,${out_path_step08L1_RSeQC},'"${out_path_step08L1_RSeQC}"',g' "$logfile"
