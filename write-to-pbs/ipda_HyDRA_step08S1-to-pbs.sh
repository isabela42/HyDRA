...
logfile=logfile_ipda_hydra08S1-to-pbs_${thislogdate}.txt
...
out_path_step08S1_RSeQC="hydra08S1_strandness-assess_RSeQC_${thislogdate}"
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; reference_gene=`grep ${path_file} ${input} | cut -f3 | sort | uniq`; echo "infer_experiment.py -r ${reference_gene} -i ${path_file}.coordsorted.bam >> ${out_path_step08S1_RSeQC}/strand-assessment_${file}.txt" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
