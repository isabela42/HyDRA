...
Description: Write and submit PBS jobs for Step 09L1 (long-reads) of the
HYDRA pipeline (HYbrid De novo RNA Assembly pipeline).

Usage: bash ipda_HyDRA_step09L1-to-pbs.sh -i "path/to/input/files" -p "PBS stem" -e "email" -m INT -c INT -w "HH:MM:SS"
...
do
    case "${flag}" in
        ...
        ?) echo script usage: bash ipda_HyDRA_step09L1-to-pbs.sh -i path/to/input/files -p PBS stem -e email -m INT -c INT -w "HH:MM:SS" >&2
           exit;;
    esac
done
...
logfile=logfile_ipda_hydra09L1-to-pbs_${thislogdate}.txt
...
out_path_step09L1_FastaSplitter="hydra09L1_size-split_FastaSplitter_${thislogdate}"

## Create output directories
mkdir -p ${out_path_step09L1_FastaSplitter}
...
date
echo "## Executing bash ipda_HyDRA_step09L1-to-pbs.sh"
...
echo
echo "## Output files saved to:       ${out_path_step09L1_FastaSplitter}"
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "python ${module_fastasplitter} -f ${path_file}.fasta  -n 20000 -p ${out_path_step09L1_FastaSplitter}/${file}" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
date ## Status of all user jobs (including HYDRA step 09L1 jobs) at
qstat -u "$user"
...
sed -i 's,${out_path_step09L1_FastaSplitter},'"${out_path_step09L1_FastaSplitter}"',g' "$logfile"
