...
Description: Write and submit PBS jobs for Step 09S1 (short-reads) of the
HYDRA pipeline (HYbrid De novo RNA Assembly pipeline).

Usage: bash ipda_HyDRA_step09S1-to-pbs.sh -i "path/to/input/files" -p "PBS stem" -e "email" -m INT -c INT -w "HH:MM:SS"

Resources used for pipeline development: -m 10 -c 1 -w "02:00:00"
...
do
    case "${flag}" in
        ...
        ?) echo script usage: bash ipda_HyDRA_step09S1-to-pbs.sh -i path/to/input/files -p PBS stem -e email -m INT -c INT -w "HH:MM:SS" >&2
           exit;;
    esac
done
...
logfile=logfile_ipda_hydra09S1-to-pbs_${thislogdate}.txt
...
out_path_step09S1_FastQC="hydra09S1_qc-ribodepleted_FastQC_${thislogdate}"

## Create output directories
mkdir -p ${out_path_step09S1_FastQC}
...
date
echo "## Executing bash ipda_HyDRA_step09S1-to-pbs.sh"
...
echo
echo "## Output files saved to:       ${out_path_step09S1_FastQC}"
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; f1="${path_file}_R1.cor.fq.gz"; f2="${path_file}_R2.cor.fq.gz"; echo "fastqc -t ${ncpus} --outdir ${out_path_step09S1_FastQC} ${f1} ${f2} --memory 10000" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
date ## Status of all user jobs (including HYDRA step 09S1 jobs) at
qstat -u "$user"
...
sed -i 's,${out_path_step09S1_FastQC},'"${out_path_step09S1_FastQC}"',g' "$logfile"
