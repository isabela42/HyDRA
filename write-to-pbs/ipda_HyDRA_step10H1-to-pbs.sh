...
logfile=logfile_ipda_hydra10H1-to-pbs_${thislogdate}.txt
...
out_path_step10H1_Bash="hydra10H1_summary_Bash_${thislogdate}"
...
step=`echo ${folder} | rev | cut -d'/' -f1 | rev | cut -d'_' -f1` ; if [[ "${step}" == "hydra01L3" ]]; then
...
sample=`echo "$(basename ${pbs_o})" | cut -d'_' -f4- | rev | cut -d'_' -f2- | rev`;
...
col05=`cut -f2 temp.txt | awk '{ a[i++]=$1; } END { if (i > 0) print a[int(i/2)]; else print "NaN" }'`; # MedianPhred
#col06=`cut -f2 temp.txt | awk 'NR == 1 { min = $1 } $1 < min { min = $1 } END { print min }'`;  # MinPhred
col06=`cut -f2 temp.txt | awk 'NR == 1 { min = $1 } NR > 1 && $1 < min { min = $1 } END { if (NR > 0) print min; else print "NaN" }'`; # MinPhred
#col07=`cut -f2 temp.txt | awk '{if (NR == 1 || $1 > max) max = $1} END {print max}'`;           # MaxPhred
col07=`cut -f2 temp.txt | awk '{if (NR == 1 || $1 > max) max = $1} END {if (NR > 0) print max; else print "NaN"}'`; # MaxPhred
...
cut -f1 ${input} | while read folder; do
step=`echo ${folder} | rev | cut -d'/' -f1 | rev | cut -d'_' -f1` ; if [[ "${step}" == "hydra01L5" ]]; then
...
cut -f1 ${input} | while read folder; do
step=`echo ${folder} | rev | cut -d'/' -f1 | rev | cut -d'_' -f1` ; if [[ "${step}" == "hydra01S1" ]]; then
...
col01="${step}";                                                                                 # Step
col02=`echo "$(basename ${pbs_o})" | rev | cut -d"_" -f2- | rev | cut -d'_' -f4-`;               # Sample
col03=`grep "Filename" fastqc_data.txt | cut -f2 | rev | cut -d"_" -f1-2 | rev | cut -d"." -f1`; # Lane_Read
...
col06=`cut -f2 temp.txt | awk '{ a[i++]=$1; } END { if (i > 0) print a[int(i/2)]; else print "NaN"; }'`;  # MedianPhred
#col07=`cut -f2 temp.txt | awk 'NR == 1 { min = $1 } $1 < min { min = $1 } END { print min }'`;   # MinPhred
col07=`cut -f2 temp.txt | awk 'NR == 1 { min = $1 } $1 < min { min = $1 } END { if (NR > 0) print min; else print "NaN" }'`;  # MinPhred
#col08=`cut -f2 temp.txt | awk '{if (NR == 1 || $1 > max) max = $1} END {print max}'`;            # MaxPhred
col08=`cut -f2 temp.txt | awk '{if (NR == 1 || $1 > max) max = $1} END {if (NR > 0) print max; else print "NaN"}'`;  # MaxPhred
col09=`echo "$(basename ${pbs_o})"`;                                                             # Base-file

echo -e "${col01}\t${col02}\t${col03}\t${col04}\t${col05}\t${col06}\t${col07}\t${col08}\t${col09}" >> ${summary_output};

header="Step\tSample\tBase\tValues\tBase-file"
summary_output2="${out_path_step10H1_Bash}/summary_${run_name}_boxplot.tsv"
echo -e "${header}" > ${summary_output2}
...
echo ${col03} | tr ' ' '\n' | while read bp; do
echo ${col04} | tr ' ' '\n' | while read c4; do
echo -e "${col01}\t${col02}\t${bp}\t${c4}\t${col05}" >> ${summary_output2};
done
done
...
cut -f1 ${input} | while read folder; do
step=`echo ${folder} | rev | cut -d'/' -f1 | rev | cut -d'_' -f1`  ; if [[ "${step}" == "hydra02L1" ]]; then
...
col01="${step}"                                                                                                     # Step
col02=`echo "$(basename ${pbs_o})" | rev | cut -d"_" -f2- | rev | cut -d'_' -f4-`;                                  # Sample
col03=`grep "reads loaded" ${pbs_o} | cut -d" " -f1 | tr -d ','`;                                                   # In-reads
if grep -q "No adapters found" ${pbs_o}; then col04=${col03}; else col04=`tail -n15 ${pbs_o} | head -n1 | cut -d"/" -f1 | tr -d ',' | tr -d ' '`; fi;                                                                                                                 # Reads-trimmed-from-start
#col05=`awk -v four="$col04" -v three="$col03" 'BEGIN {print ((four/three)*100)}'`;                                 # Perc-reads-trimmed-from-start
col05=`awk -v four="$col04" -v three="$col03" 'BEGIN {if (three == four) print 0; else print ((four/three)*100)}'`; # Perc-reads-trimmed-from-start
if grep -q "No adapters found" ${pbs_o}; then col06=0; else col06=`tail -n15 ${pbs_o} | head -n1 | cut -d"(" -f2 | tr -d ',' | cut -d' ' -f1`; fi;                                                                                                                 # bp-removed-from-start
if grep -q "No adapters found" ${pbs_o}; then col07=0; else col07=`tail -n14 ${pbs_o} | head -n1 | cut -d"/" -f1 | tr -d ',' | tr -d ' '`; fi;                                                                                                                 # Reads-trimmed-from-end
col08=`awk -v seven="$col07" -v three="$col03" 'BEGIN {if (seven == 0) print 0; else print ((seven/three)*100)}'`; # Perc-reads-trimmed-from-end
if grep -q "No adapters found" ${pbs_o}; then col09=0; else col09=`tail -n14 ${pbs_o} | head -n1 | cut -d"(" -f2 | tr -d ',' | cut -d' ' -f1`; fi;                                                                                                                 # bp-removed-from-end
if grep -q "No adapters found" ${pbs_o}; then col10=0; else col10=`tail -n8 ${pbs_o} | head -n1 | cut -d"/" -f1 | tr -d ',' | tr -d ' '`; fi;                                                                                                             # Reads-split-based-middle-adapters
col11=`ls -lh ${inpfile} | cut -d" " -f5`;                                                                          # In-size
col12=`ls -lh ${outfile} | cut -d" " -f5`;                                                                          # Out-size
#col13=`zcat -c ${inpfile} | awk 'END{print NR/4}'`;                                                                 # In-seq
col13=`zcat -c ${inpfile} 2>/dev/null | awk 'END{if (NR > 0) print NR/4; else print "NaN"}'`;                       # In-seq
#col14=`zcat -c ${outfile} | awk 'END{print NR/4}'`;                                                                 # Out-seq
col14=`zcat -c ${outfile} 2>/dev/null | awk 'END{if (NR > 0) print NR/4; else print "NaN"}'`;                       # Out-seq
...
cut -f1 ${input} | while read folder; do
step=`echo ${folder} | rev | cut -d'/' -f1 | rev | cut -d'_' -f1`  ; if [[ "${step}" == "hydra02L2" ]]; then
...
inpfile=`grep "gunzip -c" ${pbs}.pbs | cut -d" " -f3`
outfile=`grep "gunzip -c" ${pbs}.pbs | cut -d" " -f15`

col01="${step}"                                                                               # Step
col02=`echo "$(basename ${pbs_e})" | rev | cut -d"_" -f2- | rev | cut -d'_' -f4-`;            # Sample
...
col07=`zcat -c ${inpfile} 2>/dev/null | awk 'END{if (NR > 0) print NR/4; else print "NaN"}'`; # In-seq
#col08=`zcat -c ${outfile} | awk 'END{print NR/4}'`;                                           # Out-seq
col08=`zcat -c ${outfile} 2>/dev/null | awk 'END{if (NR > 0) print NR/4; else print "NaN"}'`; # Out-seq
...
cut -f1 ${input} | while read folder; do
step=`echo ${folder} | rev | cut -d'/' -f1 | rev | cut -d'_' -f1`  ; if [[ "${step}" == "hydra02L3" ]]; then
...
inpfile=`grep "Command line parameters:" ${pbs_o} | cut -d" " -f5`
outfile=`grep "Command line parameters:" ${pbs_o} | cut -d" " -f7`


col01="${step}"                                                                                     # Step
col02=`echo "$(basename ${pbs_o})" | rev | cut -d"_" -f2- | rev | cut -d'_' -f4-`;                  # Sample
col03=`tail -n6 ${pbs_o} | head -n1 | tr -s ' ' | cut -d" " -f4 | tr -d ','`;                        # In-reads
col04=`tail -n3 ${pbs_o} | head -n1 | tr -s ' ' | cut -d" " -f4 | tr -d ','`;                        # In-bases
col05=`tail -n5 ${pbs_o} | head -n1 | tr -s ' ' | cut -d" " -f5 | tr -d ','`;                        # Out-reads
col06=`tail -n5 ${pbs_o} | head -n1 | tr -s ' ' | cut -d" " -f6 | cut -d"(" -f2 | cut -d")" -f1 | tr -d '%'`;   # Perc-out-reads
col07=`tail -n1 ${pbs_o} | tr -s ' ' | cut -d" " -f4 | tr -d ','`;                                   # Out-bases
col08=`tail -n1 ${pbs_o} | tr -s ' ' | cut -d" " -f6 | cut -d"(" -f2 | cut -d")" -f1 | tr -d '%'`;   # Perc-out-bases
col09=`tail -n2 ${pbs_o} | head -n1 | tr -s ' ' | cut -d" " -f2 | tr -d ','`;                        # polyA-trimmed-bp
col10=`tail -n2 ${pbs_o} | head -n1 | tr -s ' ' | cut -d" " -f4 | cut -d"(" -f2 | cut -d")" -f1 | tr -d '%'`;  # Perc-polyA-trimmed-bp
col11=`ls -lh ${inpfile} | cut -d" " -f5`;                                                          # In-size
col12=`ls -lh ${outfile} | cut -d" " -f5`;                                                          # Out-size
#col13=`zcat -c ${inpfile} | awk 'END{print NR/4}'`;                                                 # In-seq
col13=`zcat -c ${inpfile} 2>/dev/null | awk 'END{if (NR > 0) print NR/4; else print "NaN"}'`;                   # In-seq
...
cut -f1 ${input} | while read folder; do
step=`echo ${folder} | rev | cut -d'/' -f1 | rev | cut -d'_' -f1`  ; if [[ "${step}" == "hydra02S1" ]]; then
...
col07=`awk -v six="$col06" -v four="$col04" 'BEGIN {if (four != 0) print ((six/four)*100); else print "NaN"}'`; # Perc-cor-bases
#col08=`awk -v six="$col06" -v three="$col03" 'BEGIN {print ((six/three)*100)}'`;            # Cor-bases-per-read
col08=`awk -v six="$col06" -v three="$col03" 'BEGIN {if (three != 0) print ((six/three)*100); else print "NaN"}'`; # Cor-bases-per-read
...
col13=`ls -lh ${R1cor} | cut -d" " -f5`;                                                    # R1cor-size (.fq.gz)
col14=`ls -lh ${R2raw} | cut -d" " -f5`;                                                    # R2raw-size (.fq.gz)
col15=`ls -lh ${R2cor} | cut -d" " -f5`;                                                    # R2cor-size (.fq.gz)
#col16=`cat ${R1raw} | awk 'END{print NR/4}'`;                                           # R1raw-seq for unmerged files
col16=`zcat -c ${R1raw} 2>/dev/null | awk 'END{if (NR > 0) print NR/4; else print "NaN"}'`;             # R1raw-seq
#col17=`cat ${R1cor} | awk 'END{print NR/4}'`;                                           # R1cor-seq for unmerged files
col17=`zcat -c ${R1cor} 2>/dev/null | awk 'END{if (NR > 0) print NR/4; else print "NaN"}'`;   # R1cor-seq
#col18=`cat ${R2raw} | awk 'END{print NR/4}'`;                                           # R2raw-seq for unmerged files
col18=`zcat -c ${R2raw} 2>/dev/null | awk 'END{if (NR > 0) print NR/4; else print "NaN"}'`; # R2raw-seq
#col19=`cat ${R2cor} | awk 'END{print NR/4}'`;                                           # R2cor-seq for unmerged files
col19=`zcat -c ${R2cor} 2>/dev/null | awk 'END{if (NR > 0) print NR/4; else print "NaN"}'`;   # R2cor-seq
col20="${R1raw}";                                                                           # R1raw-file
col21="${R1cor}";                                                                           # R1cor-file
col22="${R2raw}";                                                                           # R2raw-file
col23="${R2cor}";                                                                           # R2cor-file
...
cut -f1 ${input} | while read folder; do
step=`echo ${folder} | rev | cut -d'/' -f1 | rev | cut -d'_' -f1`  ; if [[ "${step}" == "hydra02S2" ]]; then
...
#col16=`cat ${R1inp} | awk 'END{print NR/4}'`;                                                                                       # R1in-seq for unmerged files
col16=`zcat -c ${R1inp} 2>/dev/null | awk 'END{if (NR > 0) print NR/4; else print "NaN"}'`;                                                         # R1in-seq
#col17=`awk 'END{print NR/4}' ${folder}/${R1out}`;                                                                                       # R1out-seq
col17=`awk 'END{if (NR > 0) print NR/4; else print "NaN"}' ${folder}/${R1out} 2>/dev/null`;                                                         # R1out-seq
#col18=`cat ${R2inp} | awk 'END{print NR/4}'`;                                                                                       # R2in-seq for unmerged files
col18=`zcat -c ${R2inp} 2>/dev/null | awk 'END{if (NR > 0) print NR/4; else print "NaN"}'`;                                                         # R2in-seq
#col19=`awk 'END{print NR/4}' ${folder}/${R2out}`;                                                                                       # R2out-seq
col19=`awk 'END{if (NR > 0) print NR/4; else print "NaN"}' ${folder}/${R2out} 2>/dev/null`;                                                         # R2out-seq
...
cut -f1 ${input} | while read folder; do
step=`echo ${folder} | rev | cut -d'/' -f1 | rev | cut -d'_' -f1` ; if [[ "${step}" == "hydra03S1" ]]; then
...
col01="${step}";                                                                                # Step
col02=`echo "$(basename ${pbs_o})" | rev | cut -d"_" -f2- | rev | cut -d'_' -f4-`;              # Sample
col03=`grep "Filename" fastqc_data.txt | cut -f2 | rev | cut -d"_" -f1 | rev | cut -d"." -f1`; # Lane_Read
col04=`grep "Total Sequences" fastqc_data.txt | cut -f2`;                                       # #Reads
col05=`cut -f2 temp.txt | awk '{s+=$1} END {print (NR ? s/NR : "NaN")}'`;                       # AveragePhred
#col06=`cut -f2 temp.txt | awk ' { a[i++]=$1; } END { print a[int(i/2)]; }'`;                    # MedianPhred
col06=`cut -f2 temp.txt | awk '{ a[i++]=$1 } END { if (i > 0) print a[int(i/2)]; else print "NaN" }'`; # MedianPhred
...
cut -f1 ${input} | while read folder; do
step=`echo ${folder} | rev | cut -d'/' -f1 | rev | cut -d'_' -f1` ; if [[ "${step}" == "hydra03L3" ]]; then
...
col05=`cut -f2 temp.txt | awk '{ a[i++]=$1 } END { if (i > 0) print a[int(i/2)]; else print "NaN" }'`; # MedianPhred
#col06=`cut -f2 temp.txt | awk 'NR == 1 { min = $1 } $1 < min { min = $1 } END { print min }'`;  # MinPhred
col06=`cut -f2 temp.txt | awk 'NR == 1 { min = $1 } $1 < min { min = $1 } END { if (NR > 0) print min; else print "NaN" }'`; # MinPhred
#col07=`cut -f2 temp.txt | awk '{if (NR == 1 || $1 > max) max = $1} END {print max}'`;           # MaxPhred
col07=`cut -f2 temp.txt | awk '{if (NR == 1 || $1 > max) max = $1} END {if (NR > 0) print max; else print "NaN"}'`; # MaxPhred
col08=`echo "$(basename ${pbs_o})"`;                                                            # Base-file
...
cut -f1 ${input} | while read folder; do
step=`echo ${folder} | rev | cut -d'/' -f1 | rev | cut -d'_' -f1`  ; if [[ "${step}" == "hydra03L5" ]]; then
...
cut -f1 ${input} | while read folder; do
step=`echo ${folder} | rev | cut -d'/' -f1 | rev | cut -d'_' -f1`  ; if [[ "${step}" == "hydra04L1" ]]; then

run_name=`cut -f1 ${input} | grep "${step}" | rev | cut -d'/' -f1 | rev` ; log_file=`cut -f2 ${input} | grep "${step}"`
err_file=`cut -f3 ${input} | grep "${step}"` ; out_file=`cut -f4 ${input} | grep "${step}"`
pbs_file=`cut -f5 ${input} | grep "${step}"`
header="Step\tSample\t$\tA\tC\tG\tT\tN\tIn-size-combined\tOut-size\tIn-files\tOut-file\tBase-file"
summary_output="${out_path_step10H1_Bash}/summary_${run_name}.tsv"
echo -e "${header}" > ${summary_output}
...
pbs=`echo ${pbs_e} | cut -d"." -f1`
infile=`tail -n1 ${pbs}.pbs | rev | cut -d" " -f21- | rev | cut -d" " -f3-`
outfile=`tail -n1 ${pbs}.pbs | rev | cut -d" " -f1 | rev`
...
col09=`ls -lh ${infile} | tr -s ' ' | cut -d" " -f5 | tr 'G' '+' | tr 'M' '+' | tr -d '\n' | rev | cut -c 2- | rev | bc`; # In-size-combined
col10=`ls -lh ${outfile} | cut -d" " -f5`;                                                              # Out-size
col11="${infile}";                                                                                      # In-files
col12="${outfile}";                                                                                     # Out-file
...
cut -f1 ${input} | while read folder; do
step=`echo ${folder} | rev | cut -d'/' -f1 | rev | cut -d'_' -f1`  ; if [[ "${step}" == "hydra04L2" ]]; then
...
pbs=`echo ${pbs_e} | cut -d"." -f1`
inpfile=`tail -n1 ${pbs}.pbs | cut -d" " -f3`
outfile=`tail -n1 ${pbs}.pbs | cut -d" " -f4`
...
cut -f1 ${input} | while read folder; do
step=`echo ${folder} | rev | cut -d'/' -f1 | rev | cut -d'_' -f1`  ; if [[ "${step}" == "hydra04L3" ]]; then
...
col08=`zcat -c ${inpreads} 2>/dev/null | awk 'END{if (NR > 0) print NR/4; else print "NaN"}'`; # In-reads-seq
#col09=`zcat -c ${outfile}.gz | awk 'END{print NR/2}'`;                                         # Out-seq
col09=`zcat -c ${outfile}.gz 2>/dev/null | awk 'END{if (NR > 0) print NR/2; else print "NaN"}'`; # Out-seq
...
cut -f1 ${input} | while read folder; do
step=`echo ${folder} | rev | cut -d'/' -f1 | rev | cut -d'_' -f1`  ; if [[ "${step}" == "hydra04S1" ]]; then
...
col21=`awk 'END{if (NR > 0) print NR/4; else print "NaN"}' ${R1inp} 2>/dev/null`;                                                           # R1in-seq
#col22=`zcat -c ${R1out} | awk 'END{print NR/4}'`;                                                                               # R1out-seq
col22=`zcat -c ${R1out} 2>/dev/null | awk 'END{if (NR > 0) print NR/4; else print "NaN"}'`;                                     # R1out-seq 
#col23=`awk 'END{print NR/4}' ${R2inp}`;                                                                                         # R2in-seq
col23=`awk 'END{if (NR > 0) print NR/4; else print "NaN"}' ${R2inp} 2>/dev/null`;                                               # R2in-seq
#col24=`zcat -c ${R2out} | awk 'END{print NR/4}'`;                                                                               # R2out-seq
col24=`zcat -c ${R2out} 2>/dev/null | awk 'END{if (NR > 0) print NR/4; else print "NaN"}'`;                                     # R2out-seq
...
cut -f1 ${input} | while read folder; do
step=`echo ${folder} | rev | cut -d'/' -f1 | rev | cut -d'_' -f1`  ; if [[ "${step}" == "hydra04S2" ]]; then
...
col25=`zcat -c ${R1inp} 2>/dev/null | awk 'END{if (NR > 0) print NR/4; else print "NaN"}'`;                                                                     # R1in-seq
#col26=`zcat -c ${R1out} | awk 'END{print NR/4}'`;                                                                                                               # R1failed-seq
col26=`zcat -c ${R1out} 2>/dev/null | awk 'END{if (NR > 0) print NR/4; else print "NaN"}'`;                                                                     # R1failed-seq
#col27=`zcat -c ${R2inp} | awk 'END{print NR/4}'`;                                                                                                               # R2in-seq
col27=`zcat -c ${R2inp} 2>/dev/null | awk 'END{if (NR > 0) print NR/4; else print "NaN"}'`;                                                                     # R2in-seq
#col28=`zcat -c ${R2out} | awk 'END{print NR/4}'`;                                                                                                               # R2failed-seq
col28=`zcat -c ${R2out} 2>/dev/null | awk 'END{if (NR > 0) print NR/4; else print "NaN"}'`;                                                                     # R2failed-seq
...
cut -f1 ${input} | while read folder; do
step=`echo ${folder} | rev | cut -d'/' -f1 | rev | cut -d'_' -f1`  ; if [[ "${step}" == "hydra05L1" ]]; then
...
cut -f1 ${input} | while read folder; do
step=`echo ${folder} | rev | cut -d'/' -f1 | rev | cut -d'_' -f1` ; if [[ "${step}" == "hydra05S1" ]]; then
...
cut -f1 ${input} | while read folder; do
step=`echo ${folder} | rev | cut -d'/' -f1 | rev | cut -d'_' -f1`  ; if [[ "${step}" == "hydra06L1" ]]; then
...
col01="${step}";                                                                                                                                     # Step
col02=`echo "$(basename ${pbs_e})" | rev | cut -d"_" -f2- | rev | cut -d'_' -f4-`;                                                                   # Sample
col03=`grep "Input:" ${pbs_e} | tr -s '\t' | cut -f2 | cut -d" " -f1`;                                                     # In-reads
col04=`grep "Input:" ${pbs_e} | tr -s '\t' | cut -f3 | cut -d" " -f1`;                                                     # In-bases
col05=`grep "Contaminants:" ${pbs_e} | tr -s '\t' | cut -f2 | cut -d" " -f1`;                                              # rRNA-reads
col06=`grep "Contaminants:" ${pbs_e} | tr -s '\t' | cut -f2 | cut -d" " -f3 | cut -d"(" -f2 | cut -d")" -f1 | tr -d '%'`;  # Perc-rRNA-reads
col07=`grep "Contaminants:" ${pbs_e} | tr -s '\t' | cut -f3 | cut -d" " -f1`;                                              # rRNA-bases
col08=`grep "Contaminants:" ${pbs_e} | tr -s '\t' | cut -f3 | cut -d" " -f3 | cut -d"(" -f2 | cut -d")" -f1 | tr -d '%'`;  # Perc-rRNA-bases
col09=`grep "Total Removed:" ${pbs_e} | tr -s '\t' | cut -f2 | cut -d" " -f1`;                                             # Total-removed-reads
col10=`grep "Total Removed:" ${pbs_e} | tr -s '\t' | cut -f2 | cut -d" " -f3 | cut -d"(" -f2 | cut -d")" -f1 | tr -d '%'`; # Perc-total-removed-read
col11=`grep "Total Removed:" ${pbs_e} | tr -s '\t' | cut -f3 | cut -d" " -f1`;                                             # Total-removed-bases
col12=`grep "Total Removed:" ${pbs_e} | tr -s '\t' | cut -f3 | cut -d" " -f3 | cut -d"(" -f2 | cut -d")" -f1 | tr -d '%'`; # Perc-total-removed-bp
col13=`grep "Result:" ${pbs_e} | tr -s '\t' | cut -f2 | cut -d" " -f1`;                                                    # Out-reads
col14=`grep "Result:" ${pbs_e} | tr -s '\t' | cut -f2 | cut -d" " -f3 | cut -d"(" -f2 | cut -d")" -f1 | tr -d '%'`;        # Perc-out-reads
col15=`grep "Result:" ${pbs_e} | tr -s '\t' | cut -f3 | cut -d" " -f1`;                                                    # Out-bases
col16=`grep "Result:" ${pbs_e} | tr -s '\t' | cut -f3 | cut -d" " -f3 | cut -d"(" -f2 | cut -d")" -f1 | tr -d '%'`;        # Perc-out-bases
col17=`if [ -f ${inp} ]; then ls -lh ${inp} | cut -d" " -f5 ; else echo "0B"; fi`;                            # In-size
col18=`if [ -f ${rna} ]; then ls -lh ${rna} | cut -d" " -f5 ; else echo "0B"; fi`;                            # rRNA-size
col19=`if [ -f ${dep} ]; then ls -lh ${dep} | cut -d" " -f5 ; else echo "0B"; fi`;                            # Ribodepleted-size
#col20=`zcat -c ${inp} | awk 'END{print NR/2}'`;                                                              # In-seq
col20=`zcat -c ${inp} 2>/dev/null | awk 'END{if (NR > 0) print NR/2; else print "NaN"}'`;                     # In-seq
#col21=`awk 'END{print NR/2}' ${rna}`;                                                                         # rRNA-seq
col21=`awk 'END{if (NR > 0) print NR/2; else print "NaN"}' ${rna} 2>/dev/null`;                               # rRNA-seq
#col22=`awk 'END{print NR/2}' ${dep}`;                                                                         # Ribodepleted-seq
col22=`awk 'END{if (NR > 0) print NR/2; else print "NaN"}' ${dep} 2>/dev/null`;                               # Ribodepleted-seq
col23="${inp}";                                                                                               # In-file
col24="${rna}";                                                                                               # rRNA-file
...
cut -f1 ${input} | while read folder; do
step=`echo ${folder} | rev | cut -d'/' -f1 | rev | cut -d'_' -f1`  ; if [[ "${step}" == "hydra06S1" ]]; then
...
col19=`ls -lh ${R1dep}.gz | cut -d" " -f5`;                                                                   # R1ribodepleted-size
col20=`ls -lh ${R2inp} | cut -d" " -f5`;                                                                      # R2in-size
col21=`ls -lh ${R2rna} | cut -d" " -f5`;                                                                      # R2rRNA-size
col22=`ls -lh ${R2dep}.gz | cut -d" " -f5`;                                                                   # R2ribodepleted-size
#col23=`zcat -c ${R1inp} | awk 'END{print NR/4}'`;                                                             # R1in-seq
col23=`zcat -c ${R1inp} 2>/dev/null | awk 'END{if (NR > 0) print NR/4; else print "NaN"}'`;                   # R1in-seq
#col24=`zcat -c ${R1rna} | awk 'END{print NR/4}'`;                                                             # R1rRNA-seq
col24=`zcat -c ${R1rna} 2>/dev/null | awk 'END{if (NR > 0) print NR/4; else print "NaN"}'`;                   # R1rRNA-seq
#col25=`zcat -c ${R1dep}.gz | awk 'END{print NR/4}'`;                                                          # R1ribodepleted-seq
col25=`zcat -c ${R1dep}.gz 2>/dev/null | awk 'END{if (NR > 0) print NR/4; else print "NaN"}'`;                # R1ribodepleted-seq
#col26=`zcat -c ${R2inp} | awk 'END{print NR/4}'`;                                                             # R2in-seq
col26=`zcat -c ${R2inp} 2>/dev/null | awk 'END{if (NR > 0) print NR/4; else print "NaN"}'`;                   # R2in-seq
#col27=`zcat -c ${R2rna} | awk 'END{print NR/4}'`;                                                             # R2rRNA-seq
col27=`zcat -c ${R2rna} 2>/dev/null | awk 'END{if (NR > 0) print NR/4; else print "NaN"}'`;                   # R2rRNA-seq
#col28=`zcat -c ${R2dep}.gz | awk 'END{print NR/4}'`;                                                          # R2ribodepleted-seq
col28=`zcat -c ${R2dep}.gz 2>/dev/null | awk 'END{if (NR > 0) print NR/4; else print "NaN"}'`;                # R2ribodepleted-seq
col29="${R1inp}";                                                                                             # R1in-file
col30="${R1rna}";                                                                                             # R1rRNA-file
col31="${R1dep}.gz";                                                                                          # R1ribodepleted-size
col32="${R2inp}";                                                                                             # R2in-file
col33="${R2rna}";                                                                                             # R2rRNA-file
col34="${R2dep}.gz";                                                                                          # R2ribodepleted-size
...
cut -f1 ${input} | while read folder; do
step=`echo ${folder} | rev | cut -d'/' -f1 | rev | cut -d'_' -f1`  ; if [[ "${step}" == "hydra07L1" ]]; then
...
cut -f1 ${input} | while read folder; do
step=`echo ${folder} | rev | cut -d'/' -f1 | rev | cut -d'_' -f1`  ; if [[ "${step}" == "hydra07S1" ]]; then
...
cut -f1 ${input} | while read folder; do
step=`echo ${folder} | rev | cut -d'/' -f1 | rev | cut -d'_' -f1`  ; if [[ "${step}" == "hydra08S1" ]]; then
...
cut -f1 ${input} | while read folder; do
step=`echo ${folder} | rev | cut -d'/' -f1 | rev | cut -d'_' -f1`  ; if [[ "${step}" == "hydra08S2" ]]; then
...
cut -f1 ${input} | while read folder; do
step=`echo ${folder} | rev | cut -d'/' -f1 | rev | cut -d'_' -f1`  ; if [[ "${step}" == "hydra09L1" ]]; then
...
cut -f1 ${input} | while read folder; do
step=`echo ${folder} | rev | cut -d'/' -f1 | rev | cut -d'_' -f1` ; if [[ "${step}" == "hydra09S1" ]]; then
