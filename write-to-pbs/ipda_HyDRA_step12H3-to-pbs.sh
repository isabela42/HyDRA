...
-i <path/to/input/files>    Input TSV files including path FROM working
                            ...
                            Col1:
                            path/from/working/dir/to/hydra11H1_assembly_rnaSPAdes_DATE/stem.fasta
...
logfile=logfile_ipda_hydra12H3-to-pbs_${thislogdate}.txt
...
out_path_step12H3_TransRate="hydra12H3_qc_TransRate_${thislogdate}"
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; transcriptome_fasta=`grep "${path_file}" ${input} | cut -f2 | sort | uniq`; echo "transrate --assembly ${path_file} --reference ${transcriptome_fasta} --threads ${ncpus} --output ${out_path_step12H3_TransRate}/${file}" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "mv ${out_path_step12H3_TransRate}/${file}/assemblies.csv ${out_path_step12H3_TransRate}/${file}/${file}_assemblies.csv" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
