...
Description: Write and submit PBS jobs for Step 12H4 (short-reads based
...
Resources used for pipeline development: -m 60 -c 32 -w "05:00:00"
...
-i <path/to/input/files>    Input TSV files including path FROM working
                            ...
                            Col2:
                            path/from/working/dir/to/hydra11H2_assembly_Trinity_DAT/insilico_read_normalization/left.norm.fq
                            of .Trinity.fasta derived files.

                            Col3:
                            path/from/working/dir/to/hydra11H2_assembly_Trinity_DAT/insilico_read_normalization/right.norm.fq
                            of .Trinity.fasta derived files.

                            Col4:
                            path/from/working/dir/to/transcriptome-reference.fasta
...
logfile=logfile_ipda_hydra12H4-to-pbs_${thislogdate}.txt
...
# <https://github.com/blahah/transrate/issues/253>
module_transrate=transrate/1.0.3
#module_transrate=conda-envs/transrate-1.0.3

# Fastq-pair
# <https://github.com/linsalrob/fastq-pair>
module_fastqpair=fastq-pair/20231003
...
out_path_step12H4_TransRate="hydra12H4_qc-and-polish_TransRate_${thislogdate}"

## Create output directories
mkdir -p ${out_path_step12H4_TransRate}
...
echo
echo "## Output files saved to:       ${out_path_step12H4_TransRate}"
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "#................................................" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "module load ${module_transrate}" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "module load ${module_fastqpair}" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "#................................................" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo 'echo "## Run Fastq_pair at" ; date ; echo' >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; leftfile=`grep "${path_file}" ${input} | cut -f2`; rightfile=`grep "${path_file}" ${input} | cut -f3`; tablesize=`awk 'END{print NR}' ${leftfile} | cut -d' ' -f1`; echo "fastq_pair -t ${tablesize} ${leftfile} ${rightfile}" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done

cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo 'echo "## Run TransRate at" ; date ; echo' >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; leftfile=`grep "${path_file}" ${input} | cut -f2`; rightfile=`grep "${path_file}" ${input} | cut -f3`; transcriptome_fasta=`grep "${path_file}" ${input} | cut -f4 | sort | uniq`; echo "transrate --assembly ${path_file} --left ${leftfile}.paired.fq --right ${rightfile}.paired.fq --reference ${transcriptome_fasta} --threads ${ncpus} --output ${out_path_step12H4_TransRate}" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done

cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo 'echo "## Rename TransRate assemblies.csv file at" ; date ; echo' >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "mv ${out_path_step12H4_TransRate}/assemblies.csv ${out_path_step12H4_TransRate}/${file}_assemblies.csv" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
sed -i 's,${module_transrate},'"${module_transrate}"',g' "$logfile"
sed -i 's,${module_fastqpair},'"${module_fastqpair}"',g' "$logfile"
sed -i 's,${out_path_step12H4_TransRate},'"${out_path_step12H4_TransRate}"',g' "$logfile"
