...
Description: Write and submit PBS jobs for Step 13H1 (hybrid) of the
...
Resources used for pipeline development: -m 250 -c 16 -w "120:00:00"
...
-i <path/to/input/files>    Input TSV files including path FROM working
                            ...
                            Col1:
                            path/from/working/dir/to/hydra11H1_assembly_rnaSPAdes_DATE/transcripts.fasta

                            Col2:
                            path/from/working/dir/to/hydra06S1_ribodepletion_BBDuk_DATE/stem_R1.cor.fq.gz
                            of all header-fixed files that were used to create the assembly,
                            seppareted by comma (same order as Col3)

                            Col3:
                            path/from/working/dir/to/hydra06S1_ribodepletion_BBDuk_DATE/stem_R2.cor.fq.gz
                            of all header-fixed files that were used to create the assembly,
                            seppareted by comma (same order as Col2)
                            
                            Col4:
                            path/from/working/dir/to/hydra06L1_ribodepletion_BBDuk_DATE/stem.fasta
                            of long-read derived fasta file that was then used to create the hybrid assembly.
                            If more than 1 fasta file was used for this purpose, then Col4 should be:
                            path/from/working/dir/to/hydra12H3_qc_TransRate_DATE/stem_merged.fasta
                            containing the merged long-read derived fasta file.
...
logfile=logfile_ipda_hydra13H1-to-pbs_${thislogdate}.txt
...
out_path_step13H1_GMAP_Bowtie2="hydra13H1_read-representation_GMAP-Bowtie2_${thislogdate}"

## Create output directories
mkdir -p ${out_path_step13H1_GMAP_Bowtie2}
...
echo
echo "## Output files saved to:       ${out_path_step13H1_GMAP_Bowtie2}"
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "module load ${module_r}" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "module load ${module_picard}" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "#................................................" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo 'echo "## Build trancriptome bowtie2 index at" ; date ; echo' >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "bowtie2-build --threads ${ncpus} ${path_file} ${out_path_step13H1_GMAP_Bowtie2}/${file}_bowtie2idx" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; leftfile=`grep "${path_file}" ${input} | cut -f2`; rightfile=`grep "${path_file}" ${input} | cut -f3`; echo "bowtie2 -p ${ncpus} -q --no-unal -k 20 -x ${out_path_step13H1_GMAP_Bowtie2}/${file}_bowtie2idx -1 ${leftfile} -2 ${rightfile} | samtools view -@${ncpus} -Sb -o ${out_path_step13H1_GMAP_Bowtie2}/${file}_transcriptome-alignment-shortreads-bowtie2.bam" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; dir_file=`echo "$(dirname "${path_file%%.*}")"`; longreads_fasta=`grep ${path_file} ${input} | cut -f4` ; echo "gmap -n 0 -t ${ncpus} -B 5 --dir=${dir_file} --gseg ${path_file} ${longreads_fasta} | samtools view -@${ncpus} -bS - >> ${out_path_step13H1_GMAP_Bowtie2}/${file}_transcriptome-alignment-longreads-gmap.bam" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "samtools merge -@${ncpus} ${out_path_step13H1_GMAP_Bowtie2}/${file}_merged_shortreads-bowtie2-and-longreads-gmap.bam ${out_path_step13H1_GMAP_Bowtie2}/${file}_transcriptome-alignment-shortreads-bowtie2.bam ${out_path_step13H1_GMAP_Bowtie2}/${file}_transcriptome-alignment-longreads-gmap.bam" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done

cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo 'echo "## Coordination sort transcriptome alignments at" ; date ; echo' >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "samtools sort -@${ncpus} ${out_path_step13H1_GMAP_Bowtie2}/${file}_merged_shortreads-bowtie2-and-longreads-gmap.bam -o ${out_path_step13H1_GMAP_Bowtie2}/${file}_merged_shortreads-bowtie2-and-longreads-gmap.coordsorted.bam" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "samtools sort -@${ncpus} ${out_path_step13H1_GMAP_Bowtie2}/${file}_transcriptome-alignment-shortreads-bowtie2.bam -o ${out_path_step13H1_GMAP_Bowtie2}/${file}_transcriptome-alignment-shortreads-bowtie2.coordsorted.bam" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "samtools sort -@${ncpus} ${out_path_step13H1_GMAP_Bowtie2}/${file}_transcriptome-alignment-longreads-gmap.bam -o ${out_path_step13H1_GMAP_Bowtie2}/${file}_transcriptome-alignment-longreads-gmap.coordsorted.bam" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done

cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo 'echo "## Build samtools index of transcriptome sorted alignments at" ; date ; echo' >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "samtools index -@${ncpus} ${out_path_step13H1_GMAP_Bowtie2}/${file}_merged_shortreads-bowtie2-and-longreads-gmap.coordsorted.bam" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "samtools index -@${ncpus} ${out_path_step13H1_GMAP_Bowtie2}/${file}_transcriptome-alignment-shortreads-bowtie2.coordsorted.bam" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "samtools index -@${ncpus} ${out_path_step13H1_GMAP_Bowtie2}/${file}_transcriptome-alignment-longreads-gmap.coordsorted.bam" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done

cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo 'echo "## Build samtools transcriptome index at" ; date ; echo' >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "samtools faidx ${path_file} -o ${out_path_step13H1_GMAP_Bowtie2}/${file}_samtoolsidx" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done

cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo 'echo "## Calculate fragment sizes at" ; date ; echo' >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "${picardtool} CollectInsertSizeMetrics I=${out_path_step13H1_GMAP_Bowtie2}/${file}_merged_shortreads-bowtie2-and-longreads-gmap.coordsorted.bam O=${out_path_step13H1_GMAP_Bowtie2}/${file}_insert-size-transcriptome-alignment-merged.txt H=${out_path_step13H1_GMAP_Bowtie2}/${file}_insert-size-merged-histogram.pdf INCLUDE_DUPLICATES=FALSE" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "${picardtool} CollectInsertSizeMetrics I=${out_path_step13H1_GMAP_Bowtie2}/${file}_transcriptome-alignment-shortreads-bowtie2.coordsorted.bam O=${out_path_step13H1_GMAP_Bowtie2}/${file}_insert-size-transcriptome-alignment-shortreads.txt H=${out_path_step13H1_GMAP_Bowtie2}/${file}_insert-size-shortreads-histogram.pdf INCLUDE_DUPLICATES=FALSE" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "${picardtool} CollectInsertSizeMetrics I=${out_path_step13H1_GMAP_Bowtie2}/${file}_transcriptome-alignment-longreads-gmap.coordsorted.bam O=${out_path_step13H1_GMAP_Bowtie2}/${file}_insert-size-transcriptome-alignment-longreads.txt H=${out_path_step13H1_GMAP_Bowtie2}/${file}_insert-size-longreads-histogram.pdf INCLUDE_DUPLICATES=FALSE" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
## Submit PBS jobs 
ls ${pbs_stem}_*${thislogdate}.pbs | while read pbs; do echo ; echo "#................................................" ; echo "# This is PBS: ${pbs}" ;  echo "#" ; echo "# main command line(s): $(tail -n28 ${pbs} | head -n1)" ; echo "#                       $(tail -n25 ${pbs} | head -n1)" ; echo "#                       $(tail -n22 ${pbs} | head -n1)" ; echo "#                       $(tail -n19 ${pbs} | head -n1)" ;  echo "#                       $(tail -n16 ${pbs} | head -n1)" ; echo "#                       $(tail -n15 ${pbs} | head -n1)" ; echo "#                       $(tail -n14 ${pbs} | head -n1)" ; echo "#                       $(tail -n11 ${pbs} | head -n1)" ; echo "#                       $(tail -n10 ${pbs} | head -n1)" ; echo "#                       $(tail -n9 ${pbs} | head -n1)" ; echo "#                       $(tail -n6 ${pbs} | head -n1)" ; echo "#                       $(tail -n3 ${pbs} | head -n1)" ;  echo "#                       $(tail -n2 ${pbs} | head -n1)" ; echo "#                       $(tail -n1 ${pbs})" ; echo "#" ; echo "# now submitting PBS" ; echo "qsub ${pbs}" ; qsub ${pbs} ; echo "#................................................" ; done
...
sed -i 's,${module_picard},'"${module_picard}"',g' "$logfile"
sed -i 's,${picardtool},'"${picardtool}"',g' "$logfile"
sed -i 's,${module_r},'"${module_r}"',g' "$logfile"
sed -i 's,${out_path_step13H1_GMAP_Bowtie2},'"${out_path_step13H1_GMAP_Bowtie2}"',g' "$logfile"
