...
Description: Write and submit PBS jobs for Step 13H2 (hybrid) of the
HYDRA pipeline (HYbrid De novo RNA Assembly pipeline).

Usage: bash ipda_HyDRA_step13H2-to-pbs.sh -i "path/to/input/files" -p "PBS stem" -e "email" -m INT -c INT -w "HH:MM:SS"

Resources used for pipeline development: -m 15 -c 15 -w "05:00:00"
...
-i <path/to/input/files>    Input TSV files including path FROM working
                            ...
                            Col1:
                            path/from/working/dir/to/hydra11H2_assembly_Trinity_DATE/stem.fasta
                            of .Trinity.fasta files.

                            Col2:
                            path/from/working/dir/to/hydra11H2_assembly_Trinity_DATE/insilico_read_normalization/left.norm.fq
                            of .Trinity.fasta derived files.

                            Col3:
                            path/from/working/dir/to/hydra11H2_assembly_Trinity_DATE/insilico_read_normalization/right.norm.fq
                            of .Trinity.fasta derived files.
...
do
    case "${flag}" in
        ...
        ?) echo script usage: bash ipda_HyDRA_step13H2-to-pbs.sh -i path/to/input/files -p PBS stem -e email -m INT -c INT -w "HH:MM:SS" >&2
           exit;;
    esac
done
...
logfile=logfile_ipda_hydra13H2-to-pbs_${thislogdate}.txt

#................................................
#  Additional information
#................................................

# If the fragment size is known and the user would like to skip the
# correspondent command line (bamPEFragmentSize), simply delete/comment
# the line and inform mean insert size below, as specified bellow:
# fragmentsize=enternumberhere

# Read more about read representation in:
...
out_path_step13H2_Bowtie2="hydra13H2_read-representation_Bowtie2_${thislogdate}"

## Create output directories
mkdir -p ${out_path_step13H2_Bowtie2}
...
date
echo "## Executing bash ipda_HyDRA_step13H2-to-pbs.sh"
...
echo
echo "## Output files saved to:       ${out_path_step13H2_Bowtie2}"
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "bowtie2-build --threads ${ncpus} ${path_file} ${out_path_step13H2_Bowtie2}/${file}_bowtie2idx" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "samtools sort -@${ncpus} ${out_path_step13H2_Bowtie2}/${file}_transcriptome-alignment-shortreads-bowtie2.bam -o ${out_path_step13H2_Bowtie2}/${file}_transcriptome-alignment-shortreads-bowtie2.coordsorted.bam" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done

cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo 'echo "## Build samtools index of transcriptome sorted alignment at" ; date ; echo' >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "samtools index -@${ncpus} ${out_path_step13H2_Bowtie2}/${file}_transcriptome-alignment-shortreads-bowtie2.coordsorted.bam" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "samtools faidx ${path_file} -o ${out_path_step13H2_Bowtie2}/${file}_samtoolsidx" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "${picardtool} CollectInsertSizeMetrics I=${out_path_step13H2_Bowtie2}/${file}_transcriptome-alignment-shortreads-bowtie2.coordsorted.bam O=${out_path_step13H2_Bowtie2}/${file}_insert-size.txt H=${out_path_step13H2_Bowtie2}/${file}_insert-size-histogram.pdf INCLUDE_DUPLICATES=FALSE" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
## Submit PBS jobs 
ls ${pbs_stem}_*${thislogdate}.pbs | while read pbs; do echo ; echo "#................................................" ; echo "# This is PBS: ${pbs}" ;  echo "#" ; echo "# main command line(s): $(tail -n16 ${pbs} | head -n1)" ; echo "#                       $(tail -n13 ${pbs} | head -n1)" ; echo "#                       $(tail -n10 ${pbs} | head -n1)" ; echo "#                       $(tail -n7 ${pbs} | head -n1)" ; echo "#                       $(tail -n4 ${pbs} | head -n1)" ; echo "#                       $(tail -n1 ${pbs})" ; echo "#" ; echo "# now submitting PBS" ; echo "qsub ${pbs}" ; qsub ${pbs} ; echo "#................................................" ; done

date ## Status of all user jobs (including HYDRA step 13H2 jobs) at
qstat -u "$user"
...
sed -i 's,${out_path_step13H2_Bowtie2},'"${out_path_step13H2_Bowtie2}"',g' "$logfile"
