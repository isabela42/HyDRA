...
Description: Write and submit PBS jobs for Step 14H1 (hybrid) of the
HYDRA pipeline (HYbrid De novo RNA Assembly pipeline).

Usage: bash ipda_HyDRA_step14H1-to-pbs.sh -i "path/to/input/files" -p "PBS stem" -e "email" -m INT -c INT -w "HH:MM:SS"

Resources used for pipeline development: -m 15 -c 16 -w "10:00:00"
...
-i <path/to/input/files>    Input TSV files including path FROM working
                            ...
                            Col1:
                            path/from/working/dir/to/hydra11H*_assembly_*_DATE/stem/stem.fasta
                            of transcripts.fasta and .Trinity.fasta files. We recommend providing both raw and polished
                            ...
                            Col3:
                            reference_gmap_db_dir
                            name of reference_GMAP_genome_db_dir. Path to this must be provided separately on Col4

                            Col4:
                            path/from/working/dir/to/
                            where the Col3 dir is located
...
do
    case "${flag}" in
        ...
        ?) echo script usage: bash ipda_HyDRA_step14H1-to-pbs.sh -i path/to/input/files -p PBS stem -e email -m INT -c INT -w "HH:MM:SS" >&2
           exit;;
    esac
done
...
logfile=logfile_ipda_hydra14H1-to-pbs_${thislogdate}.txt
...
# The 'N' tag in BAM format represents skipped region from the reference.
# So if a read doesn't have a continuous alignment or a large reference,
# the region is skipped from the alignment, and then that portion of reference
# genome will be depicted in BAM file using 'N' tag in the sixth column.
# This cannot guarantee, however, that both aligned portions of the reads
# are mapped to exons. The aligned fragments can be covering exon-intron,
# intron-exon or intron-intron regions as well. But irrespective to RNA-seq
# reads, most probably these are exon-exon.
...
out_path_step14H1_GMAP="hydra14H1_splicing-assessment_GMAP_${thislogdate}"

## Create output directories
mkdir -p ${out_path_step14H1_GMAP}
...
date
echo "## Executing bash ipda_HyDRA_step14H1-to-pbs.sh"
...
echo
echo "## Output files saved to:       ${out_path_step14H1_GMAP}"
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`grep ${path_file} ${input} | cut -f2` ; echo "#................................................" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`grep ${path_file} ${input} | cut -f2` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`grep ${path_file} ${input} | cut -f2` ; echo "module load ${module_gmap}" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`grep ${path_file} ${input} | cut -f2` ; echo "module load ${module_seqtk}" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`grep ${path_file} ${input} | cut -f2` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`grep ${path_file} ${input} | cut -f2` ; echo "#................................................" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`grep ${path_file} ${input} | cut -f2` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`grep ${path_file} ${input} | cut -f2` ; echo 'echo "## Run GMAP alignment at" ; date ; echo' >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`grep ${path_file} ${input} | cut -f2` ; reference_gmap_db=`grep "${path_file}" ${input} | cut -f3 | sort | uniq`; referencedir=`grep "${path_file}" ${input} | cut -f4 | sort | uniq`; mkdir -p  ${out_path_step14H1_GMAP}/${file} ; echo "gmap -n 0 -t ${ncpus} -B 5 --dir=${referencedir} --db=${reference_gmap_db} ${path_file} --min-intronlength=50 --max-intronlength-middle=200000 --min-identity=0.9 --format=samse >> ${out_path_step14H1_GMAP}/${file}/${file}_splicing-assessment.genome.sam" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`grep ${path_file} ${input} | cut -f2` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done

cut -f1 ${input} | sort | uniq | while read path_file; do file=`grep ${path_file} ${input} | cut -f2` ; echo 'echo "## Categorise duplicated/single alignments at" ; date ; echo' >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`grep ${path_file} ${input} | cut -f2` ; echo 'grep -v "^@"' "${out_path_step14H1_GMAP}/${file}/${file}_splicing-assessment.genome.sam | awk -v A="'"'"${out_path_step14H1_GMAP}/${file}/${file}.duplicated-alignments.sam"'"'" -v B="'"'"${out_path_step14H1_GMAP}/${file}/${file}.single-alignments.sam"'"'" '"'{count[$1]++; lines[$1,count[$1]] = $0;} END {for (key in count) {file = (count[key] == 2 ? A : B); for (i = 1; i <= count[key]; i++) print lines[key, i] > file;}}'"'" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`grep ${path_file} ${input} | cut -f2` ; echo "sed -n 'p;n' ${out_path_step14H1_GMAP}/${file}/${file}.duplicated-alignments.sam >> ${out_path_step14H1_GMAP}/${file}/${file}.spliced.sam" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`grep ${path_file} ${input} | cut -f2` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done

cut -f1 ${input} | sort | uniq | while read path_file; do file=`grep ${path_file} ${input} | cut -f2` ; echo 'echo "## Categorise transcripts at" ; date ; echo' >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`grep ${path_file} ${input} | cut -f2` ; echo "cat ${out_path_step14H1_GMAP}/${file}/${file}.single-alignments.sam | awk '\$1 ~ /@/ || (\$6 !~ /N/ && \$6 !~ /\*/)' >> ${out_path_step14H1_GMAP}/${file}/${file}.unspliced.sam" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`grep ${path_file} ${input} | cut -f2` ; echo "cat ${out_path_step14H1_GMAP}/${file}/${file}.single-alignments.sam | awk '\$1 ~ /@/ || (\$6 ~ /N/)' >> ${out_path_step14H1_GMAP}/${file}/${file}.spliced.sam" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`grep ${path_file} ${input} | cut -f2` ; echo "cat ${out_path_step14H1_GMAP}/${file}/${file}.single-alignments.sam | awk '\$1 ~ /@/ || (\$6 ~ /\*/)' >> ${out_path_step14H1_GMAP}/${file}/${file}.unaligned.sam" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`grep ${path_file} ${input} | cut -f2` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done

cut -f1 ${input} | sort | uniq | while read path_file; do file=`grep ${path_file} ${input} | cut -f2` ; echo 'echo "## Create ID inputs from GMAP alignment at" ; date ; echo' >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`grep ${path_file} ${input} | cut -f2` ; echo "grep -v "^@" ${out_path_step14H1_GMAP}/${file}/${file}.spliced.sam | cut -f1 >> ${out_path_step14H1_GMAP}/${file}/${file}.spliced.ids" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`grep ${path_file} ${input} | cut -f2` ; echo "grep -v "^@" ${out_path_step14H1_GMAP}/${file}/${file}.unspliced.sam | cut -f1 >> ${out_path_step14H1_GMAP}/${file}/${file}.unspliced.ids" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`grep ${path_file} ${input} | cut -f2` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done

cut -f1 ${input} | sort | uniq | while read path_file; do file=`grep ${path_file} ${input} | cut -f2` ; echo 'echo "## Create seq inputs from GMAP alignment at" ; date ; echo' >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`grep ${path_file} ${input} | cut -f2` ; echo "seqtk subseq ${path_file} ${out_path_step14H1_GMAP}/${file}/${file}.spliced.ids >> ${out_path_step14H1_GMAP}/${file}/${file}.spliced.fasta" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`grep ${path_file} ${input} | cut -f2` ; echo "seqtk subseq ${path_file} ${out_path_step14H1_GMAP}/${file}/${file}.unspliced.ids >> ${out_path_step14H1_GMAP}/${file}/${file}.unspliced.fasta" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
## Submit PBS jobs 
ls ${pbs_stem}_*${thislogdate}.pbs | while read pbs; do echo ; echo "#................................................" ; echo "# This is PBS: ${pbs}" ;  echo "#" ; echo "# main command line(s): $(tail -n18 ${pbs} | head -n1)" ; echo "#                       $(tail -n15 ${pbs} | head -n1)" ; echo "#                       $(tail -n14 ${pbs} | head -n1)" ; echo "#                       $(tail -n11 ${pbs} | head -n1)" ;  echo "#                       $(tail -n10 ${pbs} | head -n1)" ; echo "#                       $(tail -n9 ${pbs} | head -n1)" ; echo "#                       $(tail -n6 ${pbs} | head -n1)" ; echo "#                       $(tail -n5 ${pbs} | head -n1)" ;  echo "#                       $(tail -n2 ${pbs} | head -n1)" ; echo "#                       $(tail -n1 ${pbs})" ; echo "#" ; echo "# now submitting PBS" ; echo "qsub ${pbs}" ; qsub ${pbs} ; echo "#................................................" ; done

date ## Status of all user jobs (including HYDRA step 14H1 jobs) at
qstat -u "$user"
...
sed -i 's,${out_path_step14H1_GMAP},'"${out_path_step14H1_GMAP}"',g' "$logfile"
