...
-i <path/to/input/files>    Input TSV files including path FROM working
                            ...
                            Col1:
                            path/from/working/dir/to/hydra14H1_splicing-assessment_GMAP_DATE/stem/stem.fasta
                            ...
                            Col3:
                            path/from/working/dir/to/hydra11H2_assembly_Trinity_DATE/insilico_read_normalization/left.norm.fq.paired.fq
                            of .Trinity.fasta derived files.

                            Col4:
                            path/from/working/dir/to/hydra11H2_assembly_Trinity_DATE/insilico_read_normalization/right.norm.fq.paired.fq
                            of .Trinity.fasta derived files.
...
logfile=logfile_ipda_hydra15H2-to-pbs_${thislogdate}.txt
...
out_path_step15H2_Samtools="hydra15H2_read-support_Samtools_${thislogdate}"

## Create output directories
mkdir -p ${out_path_step15H2_Samtools}
...
echo
echo "## Output files saved to:       ${out_path_step15H2_Samtools}"
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; splice_info=`grep ${path_file} ${input} | cut -f2` ; mkdir -p  ${out_path_step15H2_Samtools}/${file}_${splice_info} ; echo "cd-hit-est -o ${out_path_step15H2_Samtools}/${file}_${splice_info}/${file}_CDhit_identity${identity}_cov${coverage}.${splice_info}.fasta -c 0.${identity} -i ${path_file} -g 1 -r 0 -aL 0.9 -aS 0.${coverage} -p 1 -d 0 -b 3 -T ${ncpus} -M ${memreqmb}" >> ${pbs_stem}_${file}_${splice_info}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; splice_info=`grep ${path_file} ${input} | cut -f2` ; echo "" >> ${pbs_stem}_${file}_${splice_info}_${thislogdate}.pbs; done
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; splice_info=`grep ${path_file} ${input} | cut -f2` ; echo "bowtie2-build --threads ${ncpus} ${out_path_step15H2_Samtools}/${file}_${splice_info}/${file}_CDhit_identity${identity}_cov${coverage}.${splice_info}.fasta ${out_path_step15H2_Samtools}/${file}_${splice_info}/${file}_${splice_info}_bt2idx" >> ${pbs_stem}_${file}_${splice_info}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; splice_info=`grep ${path_file} ${input} | cut -f2` ; echo "" >> ${pbs_stem}_${file}_${splice_info}_${thislogdate}.pbs; done
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; splice_info=`grep ${path_file} ${input} | cut -f2` ; f1=`grep ${path_file} ${input} | cut -f3` ; f2=`grep ${path_file} ${input} | cut -f4`; echo "bowtie2 -q --no-mixed --no-discordant --gbar 1000 --np 0 --end-to-end -k 200 --threads ${ncpus} -x ${out_path_step15H2_Samtools}/${file}_${splice_info}/${file}_${splice_info}_bt2idx -1 ${f1} -2 ${f2} | samtools view -@${ncpus} -bS - >> ${out_path_step15H2_Samtools}/${file}_${splice_info}/${file}_CDhit_identity${identity}_cov${coverage}_shortreads_bt2.${splice_info}.bam" >> ${pbs_stem}_${file}_${splice_info}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; splice_info=`grep ${path_file} ${input} | cut -f2` ; echo "" >> ${pbs_stem}_${file}_${splice_info}_${thislogdate}.pbs; done
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; splice_info=`grep ${path_file} ${input} | cut -f2` ; echo "samtools sort -@${ncpus} ${out_path_step15H2_Samtools}/${file}_${splice_info}/${file}_CDhit_identity${identity}_cov${coverage}_shortreads_bt2.${splice_info}.bam -o ${out_path_step15H2_Samtools}/${file}_${splice_info}/${file}_CDhit_identity${identity}_cov${coverage}_shortreads_bt2.${splice_info}.coordsorted.bam" >> ${pbs_stem}_${file}_${splice_info}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; splice_info=`grep ${path_file} ${input} | cut -f2` ; echo "rm -f ${out_path_step15H2_Samtools}/${file}_${splice_info}/${file}_CDhit_identity${identity}_cov${coverage}_shortreads_bt2.${splice_info}.bam" >> ${pbs_stem}_${file}_${splice_info}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; splice_info=`grep ${path_file} ${input} | cut -f2` ; echo "" >> ${pbs_stem}_${file}_${splice_info}_${thislogdate}.pbs; done
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; splice_info=`grep ${path_file} ${input} | cut -f2` ; echo "samtools index -@${ncpus} ${out_path_step15H2_Samtools}/${file}_${splice_info}/${file}_CDhit_identity${identity}_cov${coverage}_shortreads_bt2.${splice_info}.coordsorted.bam" >> ${pbs_stem}_${file}_${splice_info}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; splice_info=`grep ${path_file} ${input} | cut -f2` ; echo "" >> ${pbs_stem}_${file}_${splice_info}_${thislogdate}.pbs; done
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; splice_info=`grep ${path_file} ${input} | cut -f2` ; echo "samtools idxstats -@${ncpus} ${out_path_step15H2_Samtools}/${file}_${splice_info}/${file}_CDhit_identity${identity}_cov${coverage}_shortreads_bt2.${splice_info}.coordsorted.bam >> ${out_path_step15H2_Samtools}/${file}_${splice_info}/${file}_CDhit_identity${identity}_cov${coverage}_shortreads_bt2_alignment-counts.${splice_info}.tsv" >> ${pbs_stem}_${file}_${splice_info}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; splice_info=`grep ${path_file} ${input} | cut -f2` ; echo "" >> ${pbs_stem}_${file}_${splice_info}_${thislogdate}.pbs; done
...
# total=`sed '$d' ${out_path_step15H2_Samtools}/${file}_${splice_info}/${file}_CDhit_identity${identity}_cov${coverage}_shortreads_bt2_alignment-counts.${splice_info}.tsv | awk 'BEGIN {OFS="\t"} {total_reads += $3} END {print total_reads}'` ; sed '$d' ${out_path_step15H2_Samtools}/${file}_${splice_info}/${file}_CDhit_identity${identity}_cov${coverage}_shortreads_bt2_alignment-counts.${splice_info}.tsv | awk -v total_reads="${total}" 'BEGIN {OFS="\t"} {scaling_factor = total_reads / 1000000} {rpm = ($3 == 0) ? 0 : $3 / scaling_factor} {rpkm = ($2 == 0) ? 0 : (rpm == 0) ? 0 : (rpm / ($2 / 1000))} {print $1, $2, $3, $4, rpm, rpkm}' > ${out_path_step15H2_Samtools}/${file}_${splice_info}/${file}_${splice_info}_shortreads_bt2_rpm_rpkm.tsv
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; splice_info=`grep ${path_file} ${input} | cut -f2` ; echo "total=\`sed '$d' ${out_path_step15H2_Samtools}/${file}_${splice_info}/${file}_CDhit_identity${identity}_cov${coverage}_shortreads_bt2_alignment-counts.${splice_info}.tsv | awk 'BEGIN {OFS=\"\t\"} {total_reads += \$3} END {print total_reads}'\` ; sed '$d' ${out_path_step15H2_Samtools}/${file}_${splice_info}/${file}_CDhit_identity${identity}_cov${coverage}_shortreads_bt2_alignment-counts.${splice_info}.tsv | awk -v total_reads=\"\${total}\" 'BEGIN {OFS=\"\t\"} {scaling_factor = total_reads / 1000000} {rpm = (\$3 == 0) ? 0 : \$3 / scaling_factor} {rpkm = (\$2 == 0) ? 0 : (rpm == 0) ? 0 : (rpm / (\$2 / 1000))} {print \$1, \$2, \$3, \$4, rpm, rpkm}' > ${out_path_step15H2_Samtools}/${file}_${splice_info}/${file}_CDhit_identity${identity}_cov${coverage}_shortreads_bt2_alignment-counts_rpm_rpkm.${splice_info}.tsv" >> ${pbs_stem}_${file}_${splice_info}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; splice_info=`grep ${path_file} ${input} | cut -f2` ; echo "" >> ${pbs_stem}_${file}_${splice_info}_${thislogdate}.pbs; done
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; splice_info=`grep ${path_file} ${input} | cut -f2` ; high_rpkm_cutoff=`grep ${path_file} ${input} | cut -f5` ; echo "awk -v rpkm_cutoff=\"${high_rpkm_cutoff}\" 'NR>0{if (\$6 > rpkm_cutoff) print}' ${out_path_step15H2_Samtools}/${file}_${splice_info}/${file}_CDhit_identity${identity}_cov${coverage}_shortreads_bt2_alignment-counts_rpm_rpkm.${splice_info}.tsv | cut -f1 >> ${out_path_step15H2_Samtools}/${file}_${splice_info}/${file}_CDhit_identity${identity}_cov${coverage}_shortreads_RPKM_highcutoff${high_rpkm_cutoff}.${splice_info}.ids ; seqtk subseq ${out_path_step15H2_Samtools}/${file}_${splice_info}/${file}_CDhit_identity${identity}_cov${coverage}.${splice_info}.fasta ${out_path_step15H2_Samtools}/${file}_${splice_info}/${file}_CDhit_identity${identity}_cov${coverage}_shortreads_RPKM_highcutoff${high_rpkm_cutoff}.${splice_info}.ids >> ${out_path_step15H2_Samtools}/${file}_${splice_info}/${file}_CDhit_identity${identity}_cov${coverage}_shortreads_RPKM_highcutoff${high_rpkm_cutoff}.${splice_info}.fasta" >> ${pbs_stem}_${file}_${splice_info}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; splice_info=`grep ${path_file} ${input} | cut -f2` ; echo "" >> ${pbs_stem}_${file}_${splice_info}_${thislogdate}.pbs; done
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; splice_info=`grep ${path_file} ${input} | cut -f2` ; low_rpkm_cutoff=`grep ${path_file} ${input} | cut -f6` ; echo "awk -v rpkm_cutoff=\"${low_rpkm_cutoff}\" 'NR>0{if (\$6 > rpkm_cutoff) print}' ${out_path_step15H2_Samtools}/${file}_${splice_info}/${file}_CDhit_identity${identity}_cov${coverage}_shortreads_bt2_alignment-counts_rpm_rpkm.${splice_info}.tsv | cut -f1 >> ${out_path_step15H2_Samtools}/${file}_${splice_info}/${file}_CDhit_identity${identity}_cov${coverage}_shortreads_RPKM_lowcutoff${low_rpkm_cutoff}.${splice_info}.ids ; seqtk subseq ${out_path_step15H2_Samtools}/${file}_${splice_info}/${file}_CDhit_identity${identity}_cov${coverage}.${splice_info}.fasta ${out_path_step15H2_Samtools}/${file}_${splice_info}/${file}_CDhit_identity${identity}_cov${coverage}_shortreads_RPKM_lowcutoff${low_rpkm_cutoff}.${splice_info}.ids >> ${out_path_step15H2_Samtools}/${file}_${splice_info}/${file}_CDhit_identity${identity}_cov${coverage}_shortreads_RPKM_lowcutoff${low_rpkm_cutoff}.${splice_info}.fasta" >> ${pbs_stem}_${file}_${splice_info}_${thislogdate}.pbs; done
...
## Submit PBS jobs 
ls ${pbs_stem}_*${thislogdate}.pbs | while read pbs; do echo ; echo "#................................................" ; echo "# This is PBS: ${pbs}" ;  echo "#" ; echo "# main command line(s): $(tail -n26 ${pbs} | head -n1)" ; echo "#                       $(tail -n23 ${pbs} | head -n1)" ; echo "#                       $(tail -n20 ${pbs} | head -n1)" ; echo "#                       $(tail -n17 ${pbs} | head -n1)" ; echo "#                       $(tail -n16 ${pbs} | head -n1)" ;  echo "#                       $(tail -n13 ${pbs} | head -n1)" ; echo "#                       $(tail -n10 ${pbs} | head -n1)" ; echo "#                       $(tail -n7 ${pbs} | head -n1)" ; echo "#                       $(tail -n4 ${pbs} | head -n1)" ; echo "#                       $(tail -n1 ${pbs})" ; echo "#" ; echo "# now submitting PBS" ; echo "qsub ${pbs}" ; qsub ${pbs} ; echo "#................................................" ; done
...
sed -i 's,${out_path_step15H2_Samtools},'"${out_path_step15H2_Samtools}"',g' "$logfile"
