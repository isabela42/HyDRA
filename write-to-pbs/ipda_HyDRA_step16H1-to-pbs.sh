...
-i <path/to/input/files>    Input TSV files including path FROM working
                            ...
                            Col1:
                            path/from/working/dir/to/hydra15H*_read-support_Samtools_DATE/stem/stem.fasta
                            ...
                            Col5:
                            path/from/working/dir/to/ref-transcriptome.bed

                            Col6:
                            path/from/working/dir/to/lncRNAdb.bed                
...
logfile=logfile_ipda_hydra16H1-to-pbs_${thislogdate}.txt
...
out_path_step16H1_BedTools="hydra16H1_transcriptome-annotation_BedTools_${thislogdate}"
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file}" | sed 's/\(.*\)\..*/\1/')"` ; reference_gmap_db=`grep "${path_file}" ${input} | cut -f2 | sort | uniq`; referencedir=`grep "${path_file}" ${input} | cut -f3 | sort | uniq`; stem_genome=`grep "${path_file}" ${input} | cut -f4 | sort | uniq`; mkdir -p ${out_path_step16H1_BedTools}/${file} ; echo "gmap -n 1 -t ${ncpus} -B 5 --dir=${referencedir} --db=${reference_gmap_db} ${path_file}  --min-intronlength=50 --max-intronlength-middle=200000 --min-identity=0.9 --format=samse > ${out_path_step16H1_BedTools}/${file}/${file}.${stem_genome}.sam" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file}" | sed 's/\(.*\)\..*/\1/')"` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file}" | sed 's/\(.*\)\..*/\1/')"` ; stem_genome=`grep "${path_file}" ${input} | cut -f4 | sort | uniq`; echo "convert2bed-megarow --input=sam --do-not-sort --max-mem=${mem}G < ${out_path_step16H1_BedTools}/${file}/${file}.${stem_genome}.sam > ${out_path_step16H1_BedTools}/${file}/${file}.${stem_genome}.bed12" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file}" | sed 's/\(.*\)\..*/\1/')"` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file}" | sed 's/\(.*\)\..*/\1/')"` ; stem_genome=`grep "${path_file}" ${input} | cut -f4 | sort | uniq`; echo "cut -f1-6 ${out_path_step16H1_BedTools}/${file}/${file}.${stem_genome}.bed12 >> ${out_path_step16H1_BedTools}/${file}/${file}.${stem_genome}.bed" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file}" | sed 's/\(.*\)\..*/\1/')"` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file}" | sed 's/\(.*\)\..*/\1/')"` ; stem_genome=`grep "${path_file}" ${input} | cut -f4 | sort | uniq`; transcriptome=`grep "${path_file}" ${input} | cut -f5 | sort | uniq`; echo "bedtools intersect -wo -s -f 0.75 -r -a ${out_path_step16H1_BedTools}/${file}/${file}.${stem_genome}.bed -b ${transcriptome} >> ${out_path_step16H1_BedTools}/${file}/${file}.${stem_genome}.genomefeatures.bed" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file}" | sed 's/\(.*\)\..*/\1/')"` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file}" | sed 's/\(.*\)\..*/\1/')"` ; stem_genome=`grep "${path_file}" ${input} | cut -f4 | sort | uniq`; transcriptome=`grep "${path_file}" ${input} | cut -f5 | sort | uniq`; echo "bedtools intersect -s -f 0.75 -r -C -a ${out_path_step16H1_BedTools}/${file}/${file}.${stem_genome}.bed -b ${transcriptome} >> ${out_path_step16H1_BedTools}/${file}/${file}.${stem_genome}.genomecounts.bed" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file}" | sed 's/\(.*\)\..*/\1/')"` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file}" | sed 's/\(.*\)\..*/\1/')"` ; stem_genome=`grep "${path_file}" ${input} | cut -f4 | sort | uniq`; transcriptome=`grep "${path_file}" ${input} | cut -f5 | sort | uniq`; echo "bedtools intersect -v -wa -s -f 0.75 -r -a ${out_path_step16H1_BedTools}/${file}/${file}.${stem_genome}.bed -b ${transcriptome} >> ${out_path_step16H1_BedTools}/${file}/${file}.${stem_genome}.notingenome.bed" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file}" | sed 's/\(.*\)\..*/\1/')"` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file}" | sed 's/\(.*\)\..*/\1/')"` ; stem_genome=`grep "${path_file}" ${input} | cut -f4 | sort | uniq`; lncrnadb=`grep "${path_file}" ${input} | cut -f6 | sort | uniq`; echo "bedtools intersect -wo -s -f 0.75 -r -a ${out_path_step16H1_BedTools}/${file}/${file}.${stem_genome}.notingenome.bed -b ${lncrnadb} >> ${out_path_step16H1_BedTools}/${file}/${file}.${stem_genome}.lncRNAfeatures.bed" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file}" | sed 's/\(.*\)\..*/\1/')"` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file}" | sed 's/\(.*\)\..*/\1/')"` ; stem_genome=`grep "${path_file}" ${input} | cut -f4 | sort | uniq`; transcriptome=`grep "${path_file}" ${input} | cut -f5 | sort | uniq`; lncrnadb=`grep "${path_file}" ${input} | cut -f6 | sort | uniq`; echo "bedtools intersect -v -wa -s -f 0.75 -r -a ${out_path_step16H1_BedTools}/${file}/${file}.${stem_genome}.notingenome.bed -b ${lncrnadb} ${transcriptome} >> ${out_path_step16H1_BedTools}/${file}/${file}.${stem_genome}.novel.bed" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file}" | sed 's/\(.*\)\..*/\1/')"` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file}" | sed 's/\(.*\)\..*/\1/')"` ; stem_genome=`grep "${path_file}" ${input} | cut -f4 | sort | uniq`; transcriptome=`grep "${path_file}" ${input} | cut -f5 | sort | uniq`; lncrnadb=`grep "${path_file}" ${input} | cut -f6 | sort | uniq`; echo "bedtools intersect  -wo -s -f 0.5 -e -a ${out_path_step16H1_BedTools}/${file}/${file}.${stem_genome}.novel.bed -b ${lncrnadb} ${transcriptome} >> ${out_path_step16H1_BedTools}/${file}/${file}.${stem_genome}.potentialfragment.bed" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file}" | sed 's/\(.*\)\..*/\1/')"` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file}" | sed 's/\(.*\)\..*/\1/')"` ; stem_genome=`grep "${path_file}" ${input} | cut -f4 | sort | uniq`; echo "cut -f12,16 ${out_path_step16H1_BedTools}/${file}/${file}.${stem_genome}.genomefeatures.bed | tr '\"' '\t' | sort | uniq >> ${out_path_step16H1_BedTools}/${file}/${file}.${stem_genome}_geneID-type_retrieved-annotated-genes" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
# For common ensembl annotation
#cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file}" | sed 's/\(.*\)\..*/\1/')"` ; stem_genome=`grep "${path_file}" ${input} | cut -f4 | sort | uniq`; echo "cut -d'\"' -f2,14 ${out_path_step16H1_BedTools}/${file}/${file}.${stem_genome}.genomefeatures.bed | tr '\"' '\t' | sort | uniq >> ${out_path_step16H1_BedTools}/${file}/${file}.${stem_genome}_geneID-type_retrieved-annotated-genes" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
