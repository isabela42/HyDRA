...
do
    case "${flag}" in
        ...
        ?) echo script usage: bash ipda_HyDRA_step18H2-to-pbs.sh -i path/to/input/files -p PBS stem -e email -m INT -c INT -w "HH:MM:SS" >&2
           exit;;
    esac
done
...
logfile=logfile_ipda_hydra18H2-to-pbs_${thislogdate}.txt
...
out_path_step18H2_ezLncPred="hydra18H2_coding-potential_ezLncPred-CPAT_${thislogdate}"

## Create output directories
mkdir -p ${out_path_step18H2_ezLncPred}
...
date
echo "## Executing bash ipda_HyDRA_step18H2-to-pbs.sh"
...
echo
echo "## Output files saved to:       ${out_path_step18H2_ezLncPred}"
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file}" | sed 's/\(.*\)\..*/\1/')"` ; mkdir -p ${out_path_step18H2_ezLncPred}/${file} ; annotation=`grep ${path_file} ${input} | cut -f2` ; main_annotation=`echo "$(basename "${annotation}" | sed 's/\(.*\)\..*/\1/')"` ; genome_proteins=`grep "${path_file}" ${input} | cut -f3 | sort | uniq`; echo "bedtools intersect -wo -s -f 0.6 -r -a ${annotation} -b ${genome_proteins} >> ${out_path_step18H2_ezLncPred}/${file}/${main_annotation}.proteincoding.bed" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file}" | sed 's/\(.*\)\..*/\1/')"` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file}" | sed 's/\(.*\)\..*/\1/')"` ; echo "ezLncPred -i ${path_file} -o ${out_path_step18H2_ezLncPred}/${file}/${file}.CPAT CPAT -p Human" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file}" | sed 's/\(.*\)\..*/\1/')"` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file}" | sed 's/\(.*\)\..*/\1/')"` ; echo "cat ${out_path_step18H2_ezLncPred}/${file}/${file}.CPAT | tr \"'\" '\t' | cut -f2,8 | awk 'NR>0{if(\$2>=0.5){print}}' | sort | uniq >> ${out_path_step18H2_ezLncPred}/${file}/${file}.CPAT.proteincoding" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file}" | sed 's/\(.*\)\..*/\1/')"` ; echo "cat ${out_path_step18H2_ezLncPred}/${file}/${file}.CPAT | tr \"'\" '\t' | cut -f2,8 | awk 'NR>0{if(\$2<0.5){print}}' | sort | uniq >> ${out_path_step18H2_ezLncPred}/${file}/${file}.CPAT.noncoding" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file}" | sed 's/\(.*\)\..*/\1/')"` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file}" | sed 's/\(.*\)\..*/\1/')"` ; annotation=`grep ${path_file} ${input} | cut -f2` ; echo "number=\`cut -f1 ${annotation} ${out_path_step18H2_ezLncPred}/${file}/${file}.CPAT.proteincoding | tail -n +2 | sort | uniq -c | grep -c \" 2 \"\` ; echo \"${out_path_step18H2_ezLncPred}/${file}/${file}\tCPAT Annotated ptcoding classified as ptcoding\t${number}\" >> ${out_path_step18H2_ezLncPred}/${file}/${file}.CPAT.proteincodingtest" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
date ## Status of all user jobs (including HYDRA step 18H2 jobs) at
qstat -u "$user"
...
sed -i 's,${out_path_step18H2_ezLncPred},'"${out_path_step18H2_ezLncPred}"',g' "$logfile"
