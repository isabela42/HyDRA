...
do
    case "${flag}" in
        ...
        ?) echo script usage: bash ipda_HyDRA_step18H4-to-pbs.sh -i path/to/input/files -p PBS stem -e email -m INT -c INT -w "HH:MM:SS" >&2
           exit;;
    esac
done
...
logfile=logfile_ipda_hydra18H4-to-pbs_${thislogdate}.txt
...
out_path_step18H4_ezLncPred="hydra18H4_coding-potential_ezLncPred-PLEK_${thislogdate}"

## Create output directories
mkdir -p ${out_path_step18H4_ezLncPred}
...
date
echo "## Executing bash ipda_HyDRA_step18H4-to-pbs.sh"
...
echo
echo "## Output files saved to:       ${out_path_step18H4_ezLncPred}"
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file}" | sed 's/\(.*\)\..*/\1/')"` ; mkdir -p ${out_path_step18H4_ezLncPred}/${file} ; annotation=`grep ${path_file} ${input} | cut -f2` ; main_annotation=`echo "$(basename "${annotation}" | sed 's/\(.*\)\..*/\1/')"` ; genome_proteins=`grep "${path_file}" ${input} | cut -f3 | sort | uniq`; echo "bedtools intersect -wo -s -f 0.6 -r -a ${annotation} -b ${genome_proteins} >> ${out_path_step18H4_ezLncPred}/${file}/${main_annotation}.proteincoding.bed" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file}" | sed 's/\(.*\)\..*/\1/')"` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file}" | sed 's/\(.*\)\..*/\1/')"` ; echo "python ${plek} -fasta ${path_file} -out ${out_path_step18H4_ezLncPred}/${file}/${file}.PLEK PLEK -thread ${ncpus}" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
# How Maina did with PLEK model inside ezLncPred library: (above is from PLEK original publication)
#cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file}" | sed 's/\(.*\)\..*/\1/')"` ; echo 'n=`wc -l'" ${path_file} | cut -d' '"' -f1`; h=`echo "((${n}/2+1)/2)*2"|bc`; split -d -a1 -l${h}'" ${path_file} ${out_path_step18H4_ezLncPred}/${file}/${file}.PLEK_; cat ${out_path_step18H4_ezLncPred}/${file}/${file}.PLEK_0 ${out_path_step18H4_ezLncPred}/${file}/${file}.PLEK_1 >> ${out_path_step18H4_ezLncPred}/${file}/${file}.PLEK_A; cat ${out_path_step18H4_ezLncPred}/${file}/${file}.PLEK_1 ${out_path_step18H4_ezLncPred}/${file}/${file}.PLEK_0 >> ${out_path_step18H4_ezLncPred}/${file}/${file}.PLEK_B; rm -f ${out_path_step18H4_ezLncPred}/${file}/${file}.PLEK_0 ${out_path_step18H4_ezLncPred}/${file}/${file}.PLEK_1" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
#cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file}" | sed 's/\(.*\)\..*/\1/')"` ; echo "ezLncPred -i ${out_path_step18H4_ezLncPred}/${file}/${file}.PLEK_A -o ${out_path_step18H4_ezLncPred}/${file}/${file}.PLEK.PLEKa PLEK --thread ${ncpus}" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
#cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file}" | sed 's/\(.*\)\..*/\1/')"` ; echo "ezLncPred -i ${out_path_step18H4_ezLncPred}/${file}/${file}.PLEK_B -o ${out_path_step18H4_ezLncPred}/${file}/${file}.PLEK.PLEKb PLEK --thread ${ncpus}" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
#cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file}" | sed 's/\(.*\)\..*/\1/')"` ; echo "rm -f ${out_path_step18H4_ezLncPred}/${file}/${file}.PLEK_A ${out_path_step18H4_ezLncPred}/${file}/${file}.PLEK_B" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file}" | sed 's/\(.*\)\..*/\1/')"` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file}" | sed 's/\(.*\)\..*/\1/')"` ; echo "cut -f1,3 ${out_path_step18H4_ezLncPred}/${file}/${file}.PLEK* | tr -d '>' | grep \"^Coding\" | cut -f2- | sed 's/ len=/\tlen=/g' | sed 's/ path=/\tpath=/g' | sort | uniq >> ${out_path_step18H4_ezLncPred}/${file}/${file}.PLEK.proteincoding" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file}" | sed 's/\(.*\)\..*/\1/')"` ; echo "cut -f1,3 ${out_path_step18H4_ezLncPred}/${file}/${file}.PLEK* | tr -d '>' | grep \"^Non-coding\" | cut -f2- | sed 's/ len=/\tlen=/g' | sed 's/ path=/\tpath=/g' | sort | uniq >> ${out_path_step18H4_ezLncPred}/${file}/${file}.PLEK.noncoding" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file}" | sed 's/\(.*\)\..*/\1/')"` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file}" | sed 's/\(.*\)\..*/\1/')"` ; annotation=`grep ${path_file} ${input} | cut -f2` ; echo "number=\`cut -f1 ${annotation} ${out_path_step18H4_ezLncPred}/${file}/${file}.PLEK.proteincoding | tail -n +2 | sort | uniq -ci | grep -c \" 2 \"\` ; echo \"${out_path_step18H4_ezLncPred}/${file}/${file}\tPLEK Annotated ptcoding classified as ptcoding\t${number}\" >> ${out_path_step18H4_ezLncPred}/${file}/${file}.PLEK.proteincodingtest" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
#cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file}" | sed 's/\(.*\)\..*/\1/')"` ; annotation=`grep ${path_file} ${input} | cut -f2` ; echo "number=\`cut -f1 ${annotation} ${out_path_step18H4_ezLncPred}/${file}/${file}.PLEK.noncoding | tail -n +2 | sort | uniq -ci | grep -c \" 2 \"\` ; echo \"${out_path_step18H4_ezLncPred}/${file}/${file}\tPLEK Annotated ptcoding classified as noncoding\t${number}\" >> ${out_path_step18H4_ezLncPred}/${file}/${file}.PLEK.proteincodingtest" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
date ## Status of all user jobs (including HYDRA step 18H4 jobs) at
qstat -u "$user"
...
sed -i 's,${out_path_step18H4_ezLncPred},'"${out_path_step18H4_ezLncPred}"',g' "$logfile"
