...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file}" | sed 's/.*\/\([^/]*\)\..*/\1/')"` ; mkdir -p ${out_path_step20H1_Bash}/${file} ; feelnc=`grep -F ${path_file} ${input} | cut -f2 | sort | uniq` ; echo "grep -P \"^1\t\" ${feelnc} | cut -f3- >> ${out_path_step20H1_Bash}/${file}/${file}.feelncmain.noncoding" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file}" | sed 's/.*\/\([^/]*\)\..*/\1/')"` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file}" | sed 's/.*\/\([^/]*\)\..*/\1/')"` ; feelnc=`grep -F ${path_file} ${input} | cut -f2 | sort | uniq` ;  echo "cut -f1 ${out_path_step20H1_Bash}/${file}/${file}.ezlncpred.min2-models.noncoding ${out_path_step20H1_Bash}/${file}/${file}.feelncmain.noncoding | sort | uniq -i -c | grep \" 2 \" | cut -c9- >> ${out_path_step20H1_Bash}/${file}/${file}.ezlncpred-and-feelnc.noncoding" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file}" | sed 's/.*\/\([^/]*\)\..*/\1/')"` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file}" | sed 's/.*\/\([^/]*\)\..*/\1/')"` ; feelnc=`grep -F ${path_file} ${input} | cut -f2 | sort | uniq` ; echo "grep \"isBest\|^1\" ${feelnc} >> ${out_path_step20H1_Bash}/${file}/${file}.feelncmain.best; head -n1 ${feelnc} >> ${out_path_step20H1_Bash}/${file}/${file}.feelncmain.filtered; head -n1 ${feelnc} >> ${out_path_step20H1_Bash}/${file}/${file}.feelncmain.filtered-and-best; head -n1 ${feelnc} >> ${out_path_step20H1_Bash}/${file}/${file}.feelncmain.filtered-and-best.intergenic; awk 'NR==FNR{a[tolower(\$1)]; next} tolower(\$2) in a' ${out_path_step20H1_Bash}/${file}/${file}.ezlncpred-and-feelnc.noncoding ${out_path_step20H1_Bash}/${file}/${file}.feelncmain.best >> ${out_path_step20H1_Bash}/${file}/${file}.feelncmain.filtered-and-best; cat ${out_path_step20H1_Bash}/${file}/${file}.feelncmain.filtered-and-best | awk 'NR==1{print}; NR>1{if(\$7==\"intergenic\"){print}}' >> ${out_path_step20H1_Bash}/${file}/${file}.feelncmain.filtered-and-best.intergenic" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file}" | sed 's/.*\/\([^/]*\)\..*/\1/')"` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
