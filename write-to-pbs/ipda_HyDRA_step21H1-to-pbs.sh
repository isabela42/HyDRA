...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file}" | sed 's/.*\/\([^/]*\)\..*/\1/')"` ; echo "#................................................" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file}" | sed 's/.*\/\([^/]*\)\..*/\1/')"` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file}" | sed 's/.*\/\([^/]*\)\..*/\1/')"` ; echo "module load ${module_perl}" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file}" | sed 's/.*\/\([^/]*\)\..*/\1/')"` ; echo "module load ${module_htslib}" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file}" | sed 's/.*\/\([^/]*\)\..*/\1/')"` ; echo "#${gtf2gff}" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file}" | sed 's/.*\/\([^/]*\)\..*/\1/')"` ; echo "#${genestats}" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file}" | sed 's/.*\/\([^/]*\)\..*/\1/')"` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file}" | sed 's/.*\/\([^/]*\)\..*/\1/')"` ; echo "#................................................" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file}" | sed 's/.*\/\([^/]*\)\..*/\1/')"` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file}" | sed 's/.*\/\([^/]*\)\..*/\1/')"` ; echo 'echo "## Get lncRNA GTF at" ; date ; echo' >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file}" | sed 's/.*\/\([^/]*\)\..*/\1/')"` ; mkdir -p ${out_path_step21H1_Bash}/${file} ; minimap=`grep ${path_file} ${input} | cut -f2` ; echo "cat ${path_file}"' | while read transcript; do grep "gene_id \"${transcript}\";"'" ${minimap} >> ${out_path_step21H1_Bash}/${file}/${file}.gtf; done" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file}" | sed 's/.*\/\([^/]*\)\..*/\1/')"` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done

cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file}" | sed 's/.*\/\([^/]*\)\..*/\1/')"` ; echo 'echo "## Get lncRNA GFF at" ; date ; echo' >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file}" | sed 's/.*\/\([^/]*\)\..*/\1/')"` ; echo "perl ${gtf2gff} ${out_path_step21H1_Bash}/${file}/${file}.gtf >> ${out_path_step21H1_Bash}/${file}/${file}.gff" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file}" | sed 's/.*\/\([^/]*\)\..*/\1/')"` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done

cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file}" | sed 's/.*\/\([^/]*\)\..*/\1/')"` ; echo 'echo "## Get lncRNA genestats at" ; date ; echo' >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file}" | sed 's/.*\/\([^/]*\)\..*/\1/')"` ; echo "${genestats} ${out_path_step21H1_Bash}/${file}/${file}.gff >> ${out_path_step21H1_Bash}/${file}/${file}.genestats" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file}" | sed 's/.*\/\([^/]*\)\..*/\1/')"` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done

cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file}" | sed 's/.*\/\([^/]*\)\..*/\1/')"` ; echo 'echo "## Get lncRNA metrics at" ; date ; echo' >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file}" | sed 's/.*\/\([^/]*\)\..*/\1/')"` ; echo "cat ${out_path_step21H1_Bash}/${file}/${file}.genestats | while read line; do echo"' "${line}" | awk -v OFS="\t" '"'"'{ exon_sum += $3; exon_len += $4; intron_sum += $5; intron_len += $6 } END { print exon_sum, exon_len / exon_sum, intron_sum, intron_len / (intron_sum+1) }'"' >> ${out_path_step21H1_Bash}/${file}/${file}.metrics ; done" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
